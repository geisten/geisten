...
[[nodiscard]] enum geist_status geist_session_create(struct geist_model              *m,
    ...
    *out = nullptr;
    if (m == nullptr || be == nullptr) {
        geist_error_set_create_time(GEIST_E_INVALID_ARG, "geist_session_create",
                                    "model or backend is null");
        return GEIST_E_INVALID_ARG;
    }
    ...
    if (sf == nullptr) {
        geist_error_set_create_time(GEIST_E_OOM, "geist_session_create",
...
void geist_session_destroy(struct geist_session *s) {
    ...
    if (sf->arch_session != nullptr &&
        ops != nullptr && ops->session_free != nullptr) {
...
[[nodiscard]] enum geist_status geist_session_set_prompt(struct geist_session *s,
    ...
    if (gtok != nullptr) {
        ...
        size_t n_enc = 0;
        if (gtok->bos_id >= 0) { enc_ids[n_enc++] = gtok->bos_id; }
        size_t enc_n = 0;
        if (!gguf_tokenizer_encode(gtok, prompt, enc_ids + n_enc,
                                    cap - n_enc, &enc_n)) {
            void *p = enc_ids; safe_free(&p);
            snprintf(sf->err_msg, sizeof(sf->err_msg),
                     "set_prompt: gguf_tokenizer_encode failed");
        ...
        ops->prefill(sf->model->text_decoder.arch_meta, n_enc,
                      (const geist_token_t *) enc_ids);
...
geist_session_tokenize(struct geist_session *s, const char *text,
                       size_t out_capacity,
    ...
    *n_out = 0;
    ...
    if (gtok != nullptr) {
        ...
        if (enc == nullptr) { sf->err_code = GEIST_E_OOM; return GEIST_E_OOM; }
        size_t enc_n = 0;
        if (!gguf_tokenizer_encode(gtok, text, enc, out_capacity, &enc_n)) {
            void *p = enc; safe_free(&p);
            snprintf(sf->err_msg, sizeof(sf->err_msg), "tokenize: gguf encode failed");
            sf->err_code = GEIST_E_IO;
            return GEIST_E_IO;
        }
        for (size_t i = 0; i < enc_n; i++) out_ids[i] = (geist_token_t) enc[i];
        *n_out = enc_n;
        void *p = enc; safe_free(&p);
        return GEIST_OK;
    }
    ...
    if (n_ids > out_capacity) {
        safe_free((void **) &ids);
        snprintf(sf->err_msg, sizeof(sf->err_msg),
                 "tokenize: %zu tokens > out_capacity %zu", n_ids, out_capacity);
        sf->err_code = GEIST_E_INVALID_ARG;
        return GEIST_E_INVALID_ARG;
    }
    for (size_t i = 0; i < n_ids; i++) out_ids[i] = (geist_token_t) ids[i];
    *n_out = n_ids;
    safe_free((void **) &ids);
    return GEIST_OK;
}
...
const float *geist_session_peek_logits(struct geist_session *s, size_t *n_logits) {
    ...
    *n_logits = 0;
    ...
    struct geist_session_full           *sf  = as_full(s);
    const struct geist_arch_ops_decoder *ops = sf->model->text_decoder.arch_ops;
    if (ops == nullptr || ops->peek_logits == nullptr) return nullptr;
    session_attach(sf);
...
static size_t propose_drafts_ngram(const geist_token_t *history, size_t history_n,
                                    geist_token_t        seed,
                                    size_t               k_max,
                                    size_t               max_suffix,
                                    geist_token_t       *out,
                                    size_t              *out_match_L) {
    out[0] = seed;
    if (out_match_L != nullptr) *out_match_L = 0;
    if (k_max <= 1 || history_n == 0) return 1;
    ...
    if (max_suffix > 8) max_suffix = 8;
    for (size_t L = max_suffix; L >= 1; L--) {
        if (L > history_n + 1) continue;
        const size_t suffix_hist = L - 1;
        if (history_n < suffix_hist) continue;
        ...
        for (size_t pi = scan_upper; pi > 0; pi--) {
            ...
            for (size_t i = 0; i + 1 < L; i++) {
                ...
                if (history[p + i] != history[hi_idx]) { match = false; break; }
            }
            if (!match) continue;
            if (L > 1 && history[p + L - 1] != seed) continue;
            if (L == 1 && history[p] != seed) continue;

            const size_t follow_start = p + L;
            if (follow_start >= history_n) continue;  /* match at tail, no drafts */
        ...
        if (found) {
            ...
            if (out_match_L != nullptr) *out_match_L = L;
            return draft_count;
        }
    }
...
spec_fallback_single(struct geist_session *s, geist_token_t out_tokens[static 1],
                     size_t *n_out) {
    geist_token_t t;
    const enum geist_status sx = geist_session_decode_step(s, &t);
    if (sx != GEIST_OK) return sx;
    out_tokens[0] = t;
    *n_out        = 1;
    return GEIST_OK;
}
...
geist_session_decode_speculative(struct geist_session *s,
                                  ...
                                  size_t               *n_out) {
    if (s == nullptr || n_out == nullptr ||
        (history_n > 0 && history == nullptr) ||
    ...
    *n_out = 0;
    ...
    if (ops == nullptr) return GEIST_E_INVALID_STATE;

    const bool can_spec = ops->peek_next_token != nullptr &&
                          ops->verify_forward  != nullptr &&
                          ops->kv_truncate     != nullptr &&
                          ops->kv_len          != nullptr;
    if (!can_spec) return spec_fallback_single(s, out_tokens, n_out);

    session_attach(sf);
    const uint64_t t_start = monotonic_ns();
    const geist_token_t seed = ops->peek_next_token(st);
    if (seed < 0) return spec_fallback_single(s, out_tokens, n_out);

    geist_token_t drafts[GEIST_SPEC_KMAX_HARDCAP];
    size_t        match_L = 0;
    const size_t  k = propose_drafts_ngram(history, history_n, seed,
                                            k_max, /*max_suffix=*/4, drafts,
                                            &match_L);
    if (k <= 1) return spec_fallback_single(s, out_tokens, n_out);
    ...
    for (size_t i = 0; i + 1 < k; i++) {
        if (verify_out[i] != drafts[i + 1]) break;
        accepted_extras++;
    }
...
const char *geist_session_token_to_str(struct geist_session *s, geist_token_t t) {
    ...
    if (gtok != nullptr) {
        ...
        if (n >= sizeof tok_buf) n = sizeof tok_buf - 1;
        tok_buf[n] = '\0';
        return tok_buf;
    }
    struct sp_bpe_tokenizer   *tok = geist_model_internal_tokenizer(sf->model);
...
[[nodiscard]] enum geist_status geist_session_attach_audio(struct geist_session *s,
    ...
    if (sample_rate != 16000) {
        snprintf(sf->err_msg, sizeof(sf->err_msg),
                 "attach_audio: only 16 kHz PCM supported (got %d Hz)", sample_rate);
        sf->err_code = GEIST_E_UNSUPPORTED;
        return GEIST_E_UNSUPPORTED;
    }
    ...
    size_t n_soft = enc_ops->encode_pcm(enc_st, pcm_samples,
                                        n_samples, soft, max_soft);
...
[[nodiscard]] enum geist_status geist_session_attach_image(struct geist_session *s,
    ...
    if (soft == nullptr) {
        snprintf(sf->err_msg, sizeof(sf->err_msg),
                 "attach_image: soft-token buffer alloc failed");
        sf->err_code = GEIST_E_OOM;
        return GEIST_E_OOM;
    }
    const uint64_t t_enc0 = monotonic_ns();
    size_t n_soft = enc_ops->encode_image(enc_st, rgb, height, width,
                                           soft, max_soft);
    sf->total_audio_encode_ns += monotonic_ns() - t_enc0;
    if (n_soft == 0) {
        safe_free((void **) &soft);
...
[[nodiscard]] enum geist_status geist_session_attach_video(struct geist_session *s,
    ...
    if (soft == nullptr) {
        snprintf(sf->err_msg, sizeof(sf->err_msg),
                 "attach_video: soft-token buffer alloc failed");
        sf->err_code = GEIST_E_OOM;
        return GEIST_E_OOM;
    }
    const uint64_t t_enc0 = monotonic_ns();
    size_t n_soft = enc_ops->encode_video(enc_st, frames, n_frames,
                                           height, width, soft, max_soft);
    sf->total_audio_encode_ns += monotonic_ns() - t_enc0;
    if (n_soft == 0) {
        safe_free((void **) &soft);
        snprintf(sf->err_msg, sizeof(sf->err_msg),
...
[[nodiscard]] enum geist_status geist_session_pin_prefix(struct geist_session *s,
                                                          size_t                n,
                                                          const geist_token_t   ids[static n]) {
