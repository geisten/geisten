...
[[nodiscard]] geist_token_t
geist_sampler_argmax(size_t n_vocab, const float logits[static n_vocab]);

/* Temperature-scaled softmax sample (temperature > 0). When temperature
 * is exactly 0 returns argmax. */
[[nodiscard]] geist_token_t
geist_sampler_temperature(size_t n_vocab, const float logits[static n_vocab],
...
[[nodiscard]] geist_token_t
geist_sampler_top_k(size_t n_vocab, const float logits[static n_vocab],
...
[[nodiscard]] geist_token_t
geist_sampler_top_p(size_t n_vocab, const float logits[static n_vocab],
...
[[nodiscard]] geist_token_t
geist_sampler_top_k_ws(struct geist_sampler_workspace *ws,
                       const float logits[static ws->n_vocab],
                       int top_k, float temperature, struct geist_rng *rng);

[[nodiscard]] geist_token_t
geist_sampler_top_p_ws(struct geist_sampler_workspace *ws,
                       const float logits[static ws->n_vocab],
                       float top_p, float temperature, struct geist_rng *rng);
