...
static st_dtype_t parse_dtype(const char* s, size_t len) {
    for (size_t i = 0; i < DTYPE_TABLE_SIZE; i++) {
        if (strlen(DTYPE_TABLE[i].str) == len &&
            strncmp(DTYPE_TABLE[i].str, s, len) == 0) {
...
size_t st_dtype_bytes(st_dtype_t dtype) {
    for (size_t i = 0; i < DTYPE_TABLE_SIZE; i++) {
        if (DTYPE_TABLE[i].dtype == dtype) return DTYPE_TABLE[i].bytes;
    }
    return 0;
}
...
const char* st_dtype_name(st_dtype_t dtype) {
    for (size_t i = 0; i < DTYPE_TABLE_SIZE; i++) {
        if (DTYPE_TABLE[i].dtype == dtype) return DTYPE_TABLE[i].str;
    }
    return "UNKNOWN";
}
...
static int at_end(const struct cursor_t* c) { return c->p >= c->end; }
...
static int parse_string(struct cursor_t* c, const char** out, size_t* len) {
    ...
    if (at_end(c)) { c->err = "unterminated string"; return 0; }
    *out = start;
...
static int skip_value(struct cursor_t* c) {
    skip_ws(c);
    if (at_end(c)) { c->err = "EOF in skip_value"; return 0; }
    char ch = *c->p;
    if (ch == '{' || ch == '[') {
        ...
        while (!at_end(c) && depth > 0) {
            char k = *c->p;
            if (k == '"') {
                const char* dummy_str; size_t dummy_len;
                if (!parse_string(c, &dummy_str, &dummy_len)) return 0;
            } else {
                if (k == open) depth++;
...
static int parse_tensor_entry(struct cursor_t* c, struct st_tensor_t* t) {
    ...
    while (1) {
        skip_ws(c);
        if (at_end(c)) { c->err = "EOF in tensor object"; return 0; }
        if (*c->p == '}') { c->p++; break; }

        const char* key; size_t key_len;
        if (!parse_string(c, &key, &key_len)) return 0;
        if (!expect(c, ':')) return 0;
        skip_ws(c);

        if (key_len == 5 && strncmp(key, "dtype", 5) == 0) {
            const char* s; size_t l;
            if (!parse_string(c, &s, &l)) return 0;
            ...
            if (!at_end(c) && *c->p == ']') { c->p++; }
                ...
                while (1) {
                    if (t->rank >= ST_MAX_RANK) { c->err = "rank exceeds ST_MAX_RANK"; return 0; }
                    uint64_t v;
                    if (!parse_uint(c, &v)) return 0;
                    t->shape[t->rank++] = (size_t)v;
                    skip_ws(c);
                    if (at_end(c)) { c->err = "EOF in shape array"; return 0; }
                    if (*c->p == ']') { c->p++; break; }
                    if (*c->p != ',') { c->err = "expected , or ] in shape"; return 0; }
                    c->p++;
                }
            }
            saw_shape = 1;
            ...
            if (!expect(c, '[')) return 0;
            if (!parse_uint(c, &off_start)) return 0;
            if (!expect(c, ',')) return 0;
            if (!parse_uint(c, &off_end)) return 0;
            if (!expect(c, ']')) return 0;
            saw_offsets = 1;
        } else {
        ...
        skip_ws(c);
        if (at_end(c)) { c->err = "EOF after tensor field"; return 0; }
        if (*c->p == ',') { c->p++; continue; }
        if (*c->p == '}') { c->p++; break; }
...
struct st_ctx* st_open(const char* path, const char** errmsg) {
    ...
    if (fsize < 8) { close(fd); set_err(errmsg, "file too small for header"); return nullptr; }
    ...
    if (header_size > fsize - 8) {
        ...
        set_err(errmsg, "header_size exceeds file"); return nullptr;
    }
    ...
    if (!ctx->name_arena) {
        safe_free((void **) &ctx); munmap(map, fsize); close(fd);
        set_err(errmsg, "malloc name_arena"); return nullptr;
    }
    ...
    if (!expect(&c, '{')) {
        st_close(ctx); set_err(errmsg, "header is not a JSON object"); return nullptr;
    }

    while (1) {
        skip_ws(&c);
        if (at_end(&c)) { st_close(ctx); set_err(errmsg, "EOF in header"); return nullptr; }
        if (*c.p == '}') { c.p++; break; }

        const char* key; size_t key_len;
        if (!parse_string(&c, &key, &key_len)) {
            st_close(ctx); set_err(errmsg, c.err ? c.err : "parse_string failed"); return nullptr;
        }
        if (!expect(&c, ':')) {
            st_close(ctx); set_err(errmsg, "expected ':' after key"); return nullptr;
        }

        // "__metadata__" is parsed but discarded.
        if (key_len == 12 && strncmp(key, "__metadata__", 12) == 0) {
            if (!skip_value(&c)) {
                st_close(ctx); set_err(errmsg, c.err ? c.err : "skip __metadata__"); return nullptr;
            }
        } else {
            ...
            if (ctx->count == ctx->cap) {
                ...
                if (!nt) { st_close(ctx); set_err(errmsg, "alloc tensors"); return nullptr; }
                if (ctx->tensors) {
            ...
            if (!parse_tensor_entry(&c, t)) {
                st_close(ctx); set_err(errmsg, c.err ? c.err : "parse_tensor_entry"); return nullptr;
            }
            ...
            if (offset + t->nbytes > fsize - ctx->data_region) {
                st_close(ctx); set_err(errmsg, "tensor offset out of file"); return nullptr;
            }
        ...
        if (*c.p == ',') { c.p++; continue; }
        if (*c.p == '}') { c.p++; break; }
        st_close(ctx); set_err(errmsg, "expected , or } at header level"); return nullptr;
    }
...
const struct st_tensor_t* st_at(const struct st_ctx* ctx, size_t idx) {
    if (!ctx || idx >= ctx->count) return nullptr;
