...
enum {
    GGUF_VT_U8 = 0, GGUF_VT_I8 = 1, GGUF_VT_U16 = 2, GGUF_VT_I16 = 3,
    GGUF_VT_U32 = 4, GGUF_VT_I32 = 5, GGUF_VT_F32 = 6, GGUF_VT_BOOL = 7,
    GGUF_VT_STRING = 8, GGUF_VT_ARRAY = 9,
    GGUF_VT_U64 = 10, GGUF_VT_I64 = 11, GGUF_VT_F64 = 12,
};
...
struct dtype_row_t { gguf_dtype_t dt; size_t block_bytes; size_t block_elems; const char* name; };
...
static const struct dtype_row_t* dtype_lookup(gguf_dtype_t dt) {
    for (size_t i = 0; i < DTYPE_ROWS_N; i++)
        if (DTYPE_ROWS[i].dt == dt) return &DTYPE_ROWS[i];
    return nullptr;
}

size_t gguf_dtype_block_size(gguf_dtype_t dt)  { const struct dtype_row_t* r = dtype_lookup(dt); return r ? r->block_bytes : 0; }
size_t gguf_dtype_block_count(gguf_dtype_t dt) { const struct dtype_row_t* r = dtype_lookup(dt); return r ? r->block_elems : 0; }
const char* gguf_dtype_name(gguf_dtype_t dt)   { const struct dtype_row_t* r = dtype_lookup(dt); return r ? r->name      : "?"; }

size_t gguf_tensor_elem_count(const struct gguf_tensor_t* t) {
    if (!t) return 0;
    size_t n = 1;
    for (int i = 0; i < t->n_dims; i++) n *= (size_t)t->dims[i];
    return n;
}
...
[[maybe_unused]] static uint8_t  read_u8 (struct cur_t* c) { if (c->p + 1 > c->end) { c->ok = false; return 0; } return *c->p++; }
static uint32_t read_u32(struct cur_t* c) { if (c->p + 4 > c->end) { c->ok = false; return 0; } uint32_t v; memcpy(&v, c->p, 4); c->p += 4; return v; }
static uint64_t read_u64(struct cur_t* c) { if (c->p + 8 > c->end) { c->ok = false; return 0; } uint64_t v; memcpy(&v, c->p, 8); c->p += 8; return v; }
...
static const char* read_str(struct cur_t* c, size_t* out_len) {
    ...
    if (!c->ok || c->p + len > c->end) { c->ok = false; *out_len = 0; return nullptr; }
...
static bool skip_value(struct cur_t* c, uint32_t vt) {
    switch (vt) {
        case GGUF_VT_U8: case GGUF_VT_I8: case GGUF_VT_BOOL:
            c->p += 1; break;
        case GGUF_VT_U16: case GGUF_VT_I16:
            c->p += 2; break;
        case GGUF_VT_U32: case GGUF_VT_I32: case GGUF_VT_F32:
            c->p += 4; break;
        case GGUF_VT_U64: case GGUF_VT_I64: case GGUF_VT_F64:
            c->p += 8; break;
        case GGUF_VT_STRING: {
            size_t len; (void)read_str(c, &len);
            break;
        }
        case GGUF_VT_ARRAY: {
            ...
            for (uint64_t i = 0; i < n; i++) {
                if (!skip_value(c, elem_vt)) return false;
                if (!c->ok) return false;
            }
            break;
        }
    ...
    if (c->p > c->end) { c->ok = false; return false; }
    return c->ok;
}
...
static bool record_meta_kv(struct gguf_ctx *ctx, size_t slot_idx,
                            const char *key, size_t key_len,
                            struct cur_t *c, uint32_t vt) {
    ...
    if (key_len == 17 && memcmp(key, "general.alignment", 17) == 0 &&
        vt == GGUF_VT_U32 && vlen == 4) {
        uint32_t a; memcpy(&a, vp_start, 4);
...
#undef GGUF_FAIL
    ...
    if (mcount > 0) {
        ...
        if (!ctx->meta_kvs) {
            gguf_close(ctx); set_err(errmsg, "calloc meta_kvs"); return nullptr;
        }
        ctx->meta_key_arena = heap_alloc_array_aligned(char, (size_t)(c.end - c.p) + 1);
        if (!ctx->meta_key_arena) {
            gguf_close(ctx); set_err(errmsg, "malloc meta_key_arena"); return nullptr;
        }
    }
    for (uint64_t i = 0; i < mcount; i++) {
        size_t klen; const char* key = read_str(&c, &klen);
        uint32_t vt = read_u32(&c);
        if (!c.ok) { gguf_close(ctx); set_err(errmsg, "metadata KV truncated"); return nullptr; }
        if (!record_meta_kv(ctx, (size_t)i, key, klen, &c, vt) || !c.ok) {
            gguf_close(ctx); set_err(errmsg, "metadata value walk failed"); return nullptr;
        }
    }

    /* Tensor info section */
    ctx->tensors = heap_calloc_array_aligned(struct gguf_tensor_t, tcount);
    if (!ctx->tensors) { gguf_close(ctx); set_err(errmsg, "calloc tensors"); return nullptr; }
    /* Upper-bound name arena by remaining bytes in mmap. */
    ctx->name_arena = heap_alloc_array_aligned(char, (size_t)(c.end - c.p) + 1);
    if (!ctx->name_arena) { gguf_close(ctx); set_err(errmsg, "malloc name_arena"); return nullptr; }

    for (uint64_t i = 0; i < tcount; i++) {
        size_t nlen; const char* nptr = read_str(&c, &nlen);
        if (!c.ok) { gguf_close(ctx); set_err(errmsg, "tensor name truncated"); return nullptr; }
        char* name_dst = ctx->name_arena + ctx->name_arena_used;
        memcpy(name_dst, nptr, nlen); name_dst[nlen] = '\0';
        ctx->name_arena_used += nlen + 1;
        ctx->tensors[i].name = name_dst;

        uint32_t n_dims = read_u32(&c);
        if (n_dims > GGUF_MAX_DIMS) { gguf_close(ctx); set_err(errmsg, "tensor n_dims > 4"); return nullptr; }
        ctx->tensors[i].n_dims = (int)n_dims;
        for (uint32_t d = 0; d < n_dims; d++) ctx->tensors[i].dims[d] = read_u64(&c);
        ...
        if (!c.ok) { gguf_close(ctx); set_err(errmsg, "tensor info truncated"); return nullptr; }
    }
    ...
    if (ctx->data_offset > fsize) { gguf_close(ctx); set_err(errmsg, "data section past EOF"); return nullptr; }
    ...
    for (uint64_t i = 0; i < tcount; i++) {
        ...
        if (!dt) {
            ...
            t->nbytes = 0; t->data = nullptr; continue;
        }
        ...
        if (elems % dt->block_elems != 0) {
            gguf_close(ctx); set_err(errmsg, "tensor elem count not divisible by block size"); return nullptr;
        }
        t->nbytes = (elems / dt->block_elems) * dt->block_bytes;
        if (ctx->data_offset + t->offset + t->nbytes > fsize) {
            gguf_close(ctx); set_err(errmsg, "tensor data past EOF"); return nullptr;
        }
        t->data = (const uint8_t*)map + ctx->data_offset + t->offset;
    }
...
static const struct gguf_meta_kv_t* meta_find(const struct gguf_ctx* ctx, const char* key) {
    if (!ctx || !ctx->meta_kvs || !key) return nullptr;
...
const char* gguf_get_meta_string(const struct gguf_ctx* ctx, const char* key,
                                  size_t* out_len) {
    ...
    uint64_t slen; memcpy(&slen, kv->vp, 8);
    if (8 + slen > kv->vlen) {
        if (out_len) *out_len = 0;
        return nullptr;
    }
...
bool gguf_get_meta_f32(const struct gguf_ctx* ctx, const char* key, float* out) {
    const struct gguf_meta_kv_t* kv = meta_find(ctx, key);
    if (!kv || !out || kv->vt != GGUF_VT_F32 || kv->vlen != 4) return false;
    memcpy(out, kv->vp, 4);
    return true;
}
...
bool gguf_get_meta_bool(const struct gguf_ctx* ctx, const char* key, bool* out) {
    const struct gguf_meta_kv_t* kv = meta_find(ctx, key);
    if (!kv || !out || kv->vt != GGUF_VT_BOOL || kv->vlen != 1) return false;
    *out = (*kv->vp != 0);
    return true;
}

bool gguf_get_meta_array_info(const struct gguf_ctx* ctx, const char* key,
                               uint32_t* out_elem_vt, uint64_t* out_count,
                               const uint8_t** out_payload) {
    const struct gguf_meta_kv_t* kv = meta_find(ctx, key);
    if (!kv || kv->vt != GGUF_VT_ARRAY || kv->vlen < 12) return false;
    uint32_t elem_vt; memcpy(&elem_vt, kv->vp, 4);
    uint64_t count;   memcpy(&count,   kv->vp + 4, 8);
    if (out_elem_vt) *out_elem_vt = elem_vt;
    if (out_count)   *out_count   = count;
    if (out_payload) *out_payload = kv->vp + 12;
    return true;
}

size_t                  gguf_tensor_count(const struct gguf_ctx* ctx) { return ctx ? (size_t)ctx->tensor_count : 0; }
const struct gguf_tensor_t*    gguf_tensor_at(const struct gguf_ctx* ctx, size_t idx) {
    if (!ctx || idx >= ctx->tensor_count) return nullptr;
