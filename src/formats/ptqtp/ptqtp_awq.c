...
struct ptqtp_awq_ctx* ptqtp_awq_open(const char* path, const char** err) {
    ...
    if (!f) { if (err) *err = ERR_OPEN; return nullptr; }
    fseek(f, 0, SEEK_END);
    long sz = ftell(f);
    fseek(f, 0, SEEK_SET);
    if (sz < 12) { fclose(f); if (err) *err = ERR_TRUNC; return nullptr; }

    uint8_t* raw = heap_alloc_array_aligned(uint8_t, sz);
    if (!raw) { fclose(f); if (err) *err = ERR_NOMEM; return nullptr; }
    if (fread(raw, 1, (size_t)sz, f) != (size_t)sz) {
        safe_free((void **) &raw); fclose(f); if (err) *err = ERR_TRUNC; return nullptr;
    }
    fclose(f);

    if (memcmp(raw, "AWQS", 4) != 0) {
        safe_free((void **) &raw); if (err) *err = ERR_MAGIC; return nullptr;
    }
    uint32_t version = 0, n_norms = 0;
    memcpy(&version, raw + 4, 4);
    memcpy(&n_norms, raw + 8, 4);
    if (version != 1) { safe_free((void **) &raw); if (err) *err = ERR_VER; return nullptr; }

    struct ptqtp_awq_ctx* ctx = heap_calloc_array_aligned(struct ptqtp_awq_ctx, 1);
    if (!ctx) { safe_free((void **) &raw); if (err) *err = ERR_NOMEM; return nullptr; }

    /* First pass: count name + data bytes. */
    size_t pos = 12;
    size_t name_total = 0, data_total = 0;
    for (uint32_t i = 0; i < n_norms; i++) {
        if (pos + 4 > (size_t)sz) goto err_trunc;
        uint32_t nlen; memcpy(&nlen, raw + pos, 4); pos += 4;
        if (pos + nlen + 4 > (size_t)sz) goto err_trunc;
        name_total += (size_t)nlen + 1;
        pos += nlen;
        uint32_t n_in; memcpy(&n_in, raw + pos, 4); pos += 4;
        if (pos + (size_t)n_in * 4 > (size_t)sz) goto err_trunc;
    ...
    if (!ctx->name_arena || !ctx->data_arena || !ctx->entries) {
        safe_free((void **) &raw); ptqtp_awq_close(ctx);
        if (err) { *err = ERR_NOMEM; } return nullptr;
    }
    ...
    for (uint32_t i = 0; i < n_norms; i++) {
        ...
        memcpy(dst, raw + pos, nlen); dst[nlen] = '\0';
...
const float* ptqtp_awq_get(const struct ptqtp_awq_ctx* ctx, const char* name, size_t* n_out) {
    ...
    for (size_t i = 0; i < ctx->n_entries; i++) {
        if (strcmp(ctx->entries[i].name, name) == 0) {
            if (n_out) *n_out = ctx->entries[i].n_in;
