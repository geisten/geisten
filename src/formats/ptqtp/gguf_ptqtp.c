...
void ptqtp_close(struct ptqtp_ctx* ctx) {
    if (!ctx) return;
    if (ctx->map && ctx->map != MAP_FAILED) munmap((void*)ctx->map, ctx->map_size);
...
struct ptqtp_ctx* ptqtp_open(const char* path, const char** err) {
    ...
    if (!ctx) { if (err) *err = ERR_NOMEM; return nullptr; }
    ctx->fd = -1;

    ctx->fd = open(path, O_RDONLY);
    if (ctx->fd < 0) { if (err) *err = ERR_OPEN; ptqtp_close(ctx); return nullptr; }

    struct stat st;
    if (fstat(ctx->fd, &st) < 0) { if (err) *err = ERR_FSTAT; ptqtp_close(ctx); return nullptr; }
    ctx->map_size = (size_t)st.st_size;

    void* m = mmap(nullptr, ctx->map_size, PROT_READ, MAP_PRIVATE, ctx->fd, 0);
    if (m == MAP_FAILED) { if (err) *err = ERR_MMAP; ptqtp_close(ctx); return nullptr; }
    ctx->map = (const uint8_t*)m;

    /* Parse header: 4 magic + 4*4 fields = 20 bytes */
    if (ctx->map_size < 20) { if (err) *err = ERR_TRUNC; ptqtp_close(ctx); return nullptr; }
    if (memcmp(ctx->map, "PTQT", 4) != 0) { if (err) *err = ERR_MAGIC; ptqtp_close(ctx); return nullptr; }
    ...
    if (ctx->version != 1 && ctx->version != 2) {
        if (err) { *err = ERR_VERSION; } ptqtp_close(ctx); return nullptr;
    }
    if (ctx->n_planes != 2 && ctx->n_planes != 3) {
        if (err) { *err = ERR_NPLANES; } ptqtp_close(ctx); return nullptr;
    }
    ...
    size_t arena_bytes = 0;
    {
        ...
        for (uint32_t i = 0; i < ctx->n_tensors; i++) {
            ...
            uint32_t nlen; memcpy(&nlen, q, 4); q += 4;
    ...
    /* Second pass: populate tensor table. */
    {
        ...
        for (uint32_t i = 0; i < ctx->n_tensors; i++) {
            ...
            q += 32;
            memcpy(&ctx->tensors[i].cos_sim, q, 4); q += 4;
            ...
            if (trit_off + trit_sz > ctx->map_size ||
                alpha_off + alpha_sz > ctx->map_size) {
                if (err) *err = ERR_TRUNC;
                ptqtp_close(ctx);
                return nullptr;
            }
    ...
    }
    {
        size_t off = 0;
        for (uint32_t i = 0; i < ctx->n_tensors; i++) {
            const size_t n = (size_t)ctx->tensors[i].n_out
                              * ctx->tensors[i].n_groups
                              * ctx->tensors[i].n_planes;
...
size_t   ptqtp_n_tensors(const struct ptqtp_ctx* ctx)  { return ctx ? ctx->n_tensors : 0; }
uint32_t ptqtp_group_size(const struct ptqtp_ctx* ctx) { return ctx ? ctx->group_size : 0; }
uint32_t ptqtp_n_planes (const struct ptqtp_ctx* ctx)  { return ctx ? ctx->n_planes : 0; }

bool ptqtp_is_mixed(const struct ptqtp_ctx* ctx) {
    if (!ctx) return false;
    const uint8_t header_planes = (uint8_t)ctx->n_planes;
    for (uint32_t i = 0; i < ctx->n_tensors; i++) {
        if (ctx->tensors[i].n_planes != header_planes) return true;
    }
    return false;
}
