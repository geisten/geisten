...
bool gguf_dequant_row_to_fp32(const struct gguf_tensor_t* t, size_t row_idx,
    ...
    switch (t->dtype) {
        ...
        case GGUF_TYPE_Q3_K: {
            ...
            const uint8_t* base = (const uint8_t*)t->data
                                  + row_idx * blocks_per_row * Q3_K_BLOCK_BYTES;
        ...
        case GGUF_TYPE_Q4_K: {
            ...
            const uint8_t* base = (const uint8_t*)t->data
                                  + row_idx * blocks_per_row * Q4_K_BLOCK_BYTES;
        ...
        case GGUF_TYPE_Q5_K: {
            ...
            const uint8_t* base = (const uint8_t*)t->data
                                  + row_idx * blocks_per_row * Q5_K_BLOCK_BYTES;
        ...
        case GGUF_TYPE_Q6_K: {
            ...
            const uint8_t* base = (const uint8_t*)t->data
                                  + row_idx * blocks_per_row * Q6_K_BLOCK_BYTES;
        ...
        case GGUF_TYPE_IQ2_S: {
            ...
            const uint8_t* base = (const uint8_t*)t->data
                                  + row_idx * blocks_per_row * IQ2_S_BLOCK_BYTES;
        ...
        case GGUF_TYPE_IQ3_S: {
            const size_t blocks_per_row = row_elems / 256;
            const uint8_t* base = (const uint8_t*)t->data
                                  + row_idx * blocks_per_row * IQ3_S_BLOCK_BYTES;
        ...
        case GGUF_TYPE_TQ2_0: {
            ...
            const uint8_t* base = (const uint8_t*)t->data
                                  + row_idx * blocks_per_row * TQ2_0_BLOCK_BYTES;
...
float* gguf_dequant_to_fp32(const struct gguf_tensor_t* t) {
    ...
    switch (t->dtype) {
        ...
        case GGUF_TYPE_F16: {
            ...
            for (size_t i = 0; i < elems; i++) out[i] = fp16_to_fp32(h[i]);
            break;
        }
        case GGUF_TYPE_BF16: {
            ...
            for (size_t i = 0; i < elems; i++) o[i] = ((uint32_t)h[i]) << 16;
            break;
        }
        case GGUF_TYPE_Q8_0:  dequant_q8_0_row (t->data, out, elems); break;
        case GGUF_TYPE_Q3_K:  dequant_q3_K_row (t->data, out, elems); break;
        case GGUF_TYPE_Q4_K:  dequant_q4_K_row (t->data, out, elems); break;
        case GGUF_TYPE_Q5_K:  dequant_q5_K_row (t->data, out, elems); break;
        case GGUF_TYPE_Q6_K:  dequant_q6_K_row (t->data, out, elems); break;
        case GGUF_TYPE_IQ2_S: dequant_iq2_s_row(t->data, out, elems); break;
        case GGUF_TYPE_IQ3_S: dequant_iq3_s_row(t->data, out, elems); break;
        case GGUF_TYPE_TQ2_0: dequant_tq2_0_row(t->data, out, elems); break;
        default:
            safe_free((void **) &out);
            return nullptr;
    }
...
float quantize_x_int8_sym(const float* x, size_t n, int8_t* x_q8) {
...
#else
    float amax = 0.0f;
    for (size_t i = 0; i < n; i++) { float a = fabsf(x[i]); if (a > amax) amax = a; }
#endif
    ...
    for (; j < n; j++) x_q8[j] = (int8_t)lrintf(x[j] * inv);
#else
    /* |x[i]·inv| ≤ 127 by construction (inv = 127/amax), so cast is in-range. */
    for (size_t i = 0; i < n; i++) x_q8[i] = (int8_t)lrintf(x[i] * inv);
#endif
    return scale;
}
