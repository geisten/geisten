    ...
    for (int ib32 = 0; ib32 < 8; ib32 += 2) {
        ...
        for (int l = 0; l < 4; l++) {
            const uint8_t* grid1 = (const uint8_t*)(iq3s_grid +
                ((uint32_t)qs[2*l + 0] | ((uint32_t)(qh[ib32 + 0] << (8 - 2*l)) & 256)));
            const uint8_t* grid2 = (const uint8_t*)(iq3s_grid +
                ((uint32_t)qs[2*l + 1] | ((uint32_t)(qh[ib32 + 0] << (7 - 2*l)) & 256)));
            for (int j = 0; j < 4; j++) {
                y[j + 0] = db1 * (float)grid1[j] * ((signs[l] & kmask_iq2xs[j + 0]) ? -1.0f : 1.0f);
                y[j + 4] = db1 * (float)grid2[j] * ((signs[l] & kmask_iq2xs[j + 4]) ? -1.0f : 1.0f);
            }
            y += 8;
        }
        qs += 8;
        ...
        for (int l = 0; l < 4; l++) {
            const uint8_t* grid1 = (const uint8_t*)(iq3s_grid +
                ((uint32_t)qs[2*l + 0] | ((uint32_t)(qh[ib32 + 1] << (8 - 2*l)) & 256)));
            const uint8_t* grid2 = (const uint8_t*)(iq3s_grid +
                ((uint32_t)qs[2*l + 1] | ((uint32_t)(qh[ib32 + 1] << (7 - 2*l)) & 256)));
            for (int j = 0; j < 4; j++) {
                y[j + 0] = db2 * (float)grid1[j] * ((signs[l] & kmask_iq2xs[j + 0]) ? -1.0f : 1.0f);
                y[j + 4] = db2 * (float)grid2[j] * ((signs[l] & kmask_iq2xs[j + 4]) ? -1.0f : 1.0f);
            }
            y += 8;
        }
        qs += 8;
...
void dequant_iq3_s_row(const void* blocks, float* out, size_t n_elems) {
    ...
    for (size_t i = 0; i < nb; i++) dequant_iq3_s_block(&b[i], out + i * 256);
}
