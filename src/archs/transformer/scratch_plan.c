...
void transformer_scratch_plan_build(
    ...
    const size_t M = (st->sess != NULL && st->sess->m_max > 0)
                         ? st->sess->m_max
                         : st->m_max;
    ...
    out->pool_bytes =
        out->hidden /*normed*/ + out->q_out /*q*/ +
        out->kv_out /*k*/ + out->kv_out /*v*/ +
        out->q_out /*attn*/ +
        out->hidden * 10 /*o, post_attn, h_post_attn, pre_ff,
                           ffn_out, post_ff, h_post_ff,
                           proj_ple, h_a, h_b*/ +
        out->inter * 2 /*gate, up*/ +
        out->hidden_per /*gate_ple*/ +
        out->ple_out * 2 /*ple_lookup, per_layer_input*/ +
        out->vocab /*logits*/ +
