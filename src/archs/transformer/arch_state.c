...
static void release_weight_aux(struct geist_weight *w) {
    ...
    if ((w->flags & GEIST_W_AUX_HEAP_OWNED) != 0) {
        ...
        w->aux_fp32 = nullptr;
        w->aux_n    = 0;
        w->flags   &= (uint16_t) ~(GEIST_W_AUX_HEAP_OWNED |
                                   GEIST_W_AUX_BACKEND_REPACK);
...
[[nodiscard]] static enum geist_status alloc_scratch(struct geist_backend *be,
                                                      size_t bytes,
                                                      struct geist_buffer **out) {
    enum geist_status s = be->desc->vtbl->buffer_create(
        be, bytes, GEIST_BUFFER_SCRATCH, GEIST_MEMORY_AUTO, out);
    if (s != GEIST_OK) {
        return s;
    }
 ...
 * scratch_pool_base. */
[[nodiscard]] static enum geist_status alloc_pool_buffer(
    struct transformer_arch_state *st, size_t bytes,
    ...
    if (aligned + bytes > st->sess->scratch_pool_bytes) {
        geist_backend_set_error(be, GEIST_E_OOM,
                                ...
                                st->sess->scratch_pool_used, bytes,
...
[[nodiscard]] static enum geist_status upload_table(struct geist_backend *be,
    ...
    return be->desc->vtbl->buffer_upload(*out, n_floats * sizeof(float),
                                          (const uint8_t *) src);
}
 ...
 * indexes into it. */
[[nodiscard]] static enum geist_status allocate_runtime_rope(
    ...
    for (size_t i = 0; i < st->n_layers; i++) {
        if (full_idx == -1 && st->layers[i].is_full) full_idx = (int) i;
        if (sliding_idx == -1 && !st->layers[i].is_full) sliding_idx = (int) i;
    }
    if (full_idx == -1) full_idx = 0;
    if (sliding_idx == -1) sliding_idx = full_idx;
    ...
    if (cos_sl == nullptr || sin_sl == nullptr ||
        cos_fl == nullptr || sin_fl == nullptr) {
        void *p[4] = {cos_sl, sin_sl, cos_fl, sin_fl};
        for (size_t i = 0; i < 4; i++) { safe_free(&p[i]); }
        geist_backend_set_error(be, GEIST_E_OOM,
                                "transformer: RoPE table host alloc failed");
        return GEIST_E_OOM;
    }
    rope_compute_at(0, st->max_seq_len, sliding_hd, sliding_rot,
                     sliding_th, cos_sl, sin_sl);
    rope_compute_at(0, st->max_seq_len, full_hd, full_rot,
                     full_th, cos_fl, sin_fl);

    enum geist_status s = upload_table(be, cos_sl, n_sl_floats, &st->rope_cos_sliding);
    if (s == GEIST_OK) { s = upload_table(be, sin_sl, n_sl_floats, &st->rope_sin_sliding); }
    if (s == GEIST_OK) { s = upload_table(be, cos_fl, n_fl_floats, &st->rope_cos_full); }
    if (s == GEIST_OK) { s = upload_table(be, sin_fl, n_fl_floats, &st->rope_sin_full); }

    void *to_free[4] = {cos_sl, sin_sl, cos_fl, sin_fl};
    for (size_t i = 0; i < 4; i++) { safe_free(&to_free[i]); }
    return s;
}
...
[[nodiscard]] static enum geist_status allocate_runtime_session(
    struct transformer_arch_state *st) {
    ...
    s = alloc_pool_buffer(st, scratch_plan.hidden, &st->sess->scratch_normed);      if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.q_out,  &st->sess->scratch_q);           if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.kv_out, &st->sess->scratch_k);           if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.kv_out, &st->sess->scratch_v);           if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.q_out,  &st->sess->scratch_attn);        if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.hidden, &st->sess->scratch_o);           if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.hidden, &st->sess->scratch_post_attn);   if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.hidden, &st->sess->scratch_h_post_attn); if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.hidden, &st->sess->scratch_pre_ff);      if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.inter,  &st->sess->scratch_gate);        if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.inter,  &st->sess->scratch_up);          if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.hidden, &st->sess->scratch_ffn_out);     if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.hidden, &st->sess->scratch_post_ff);     if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.hidden, &st->sess->scratch_h_post_ff);   if (s != GEIST_OK) { return s; }
    ...
    if (st->config.has_ple) {
        s = alloc_pool_buffer(st, scratch_plan.hidden_per,&st->sess->scratch_gate_ple); if (s != GEIST_OK) { return s; }
        s = alloc_pool_buffer(st, scratch_plan.hidden,    &st->sess->scratch_proj_ple); if (s != GEIST_OK) { return s; }
        ...
        s = alloc_pool_buffer(st, scratch_plan.ple_out,   &st->sess->scratch_ple_lookup); if (s != GEIST_OK) { return s; }
    ...
    s = alloc_pool_buffer(st, scratch_plan.hidden, &st->sess->scratch_h_a); if (s != GEIST_OK) { return s; }
    s = alloc_pool_buffer(st, scratch_plan.hidden, &st->sess->scratch_h_b); if (s != GEIST_OK) { return s; }
    ...
    frame_arena_init(&st->sess->scratch_arena, st->sess->scratch_arena_base,
                      st->sess->scratch_arena_bytes);
...
transformer_state_create_from_gguf(struct geist_backend            *be,
    ...
    *out = nullptr;
    ...
    if (st == nullptr) {
        gguf_close(gguf);
        geist_backend_set_error(be, GEIST_E_OOM,
                                "transformer: state alloc failed (%zu bytes)", sizeof(*st));
        return GEIST_E_OOM;
    }
    ...
    {   /* GEIST_M_MAX override — for tuning the prefill activation tile vs L1
        ...
        if (mm != nullptr && mm[0] != '\0') {
            const int v = atoi(mm);
            if (v > 0 && v <= (int) GEIST_QUANT_M_CAP) st->m_max = (size_t) v;
        }
    }
    ...
    st->layers = heap_alloc_aligned(
        st->n_layers * sizeof(*st->layers), alignof(struct transformer_layer_weights));
    if (st->layers == nullptr) {
        void *p = st; safe_free(&p);
        gguf_close(gguf);
        geist_backend_set_error(be, GEIST_E_OOM,
                                "transformer: layer array alloc failed "
                                "(%zu layers × %zu bytes)",
                                st->n_layers, sizeof(*st->layers));
        return GEIST_E_OOM;
    }
    ...
    if (!mmap_alias_mode) {
        ...
        if (st->weight_arena == nullptr) {
            geist_backend_set_error(be, GEIST_E_OOM,
                                    "transformer: weight arena alloc failed (%zu bytes)",
                                    cap);
            transformer_state_destroy(st);
            return GEIST_E_OOM;
        }
...
void transformer_state_destroy(struct transformer_arch_state *st) {
    ...
    if (be != nullptr) {
        ...
        struct geist_buffer *rope[] = {
            st->rope_cos_sliding, st->rope_sin_sliding,
            st->rope_cos_full,    st->rope_sin_full,
        };
        for (size_t i = 0; i < sizeof rope / sizeof rope[0]; i++) {
            if (rope[i] != nullptr) { be->desc->vtbl->buffer_destroy(be, rope[i]); }
        }
    }
    if (st->gguf != nullptr) {
        gguf_close((struct gguf_ctx *) st->gguf);
...
transformer_session_alloc(struct transformer_arch_state *state,
    ...
    struct transformer_arch_session *sess = heap_alloc_aligned(
        sizeof(*sess), alignof(struct transformer_arch_session));
    if (sess == nullptr) {
        geist_backend_set_error(be, GEIST_E_OOM,
                                "transformer_session_alloc: %zu bytes failed",
                                sizeof(*sess));
        return nullptr;
    }
    ...
    if (sess->m_max == 0 || sess->m_max > GEIST_QUANT_M_CAP) {
        geist_backend_set_error(be, GEIST_E_INVALID_ARG,
                                ...
                                sess->m_max, (size_t) GEIST_QUANT_M_CAP);
        void *p_sess = sess;
        safe_free(&p_sess);
        return nullptr;
    }
    ...
    struct geist_buffer **kv_block = heap_alloc_aligned(
        kv_bytes, alignof(struct geist_buffer *));
    if (kv_block == nullptr) {
        geist_backend_set_error(be, GEIST_E_OOM,
                                "transformer_session_alloc: KV slot array "
                                "alloc failed (%zu × %zu bytes)",
                                kv_slots, n_layers * sizeof(struct geist_buffer *));
        void *p_sess = sess; safe_free(&p_sess);
        return nullptr;
    }
    ...
    if (be != nullptr) {
        for (size_t li = 0; li < (size_t) state->n_layers; li++) {
            struct geist_buffer *kv_bufs[] = {
                sess->k_cache[li],        sess->v_cache[li],
                sess->k_cache_q8[li],     sess->v_cache_q8[li],
                sess->k_cache_scale[li],  sess->v_cache_scale[li],
                sess->k_kivi_q[li],       sess->v_kivi_q[li],
                sess->k_kivi_scales[li],  sess->k_kivi_zeros[li],
                sess->v_kivi_scales[li],  sess->v_kivi_zeros[li],
                sess->k_residual[li],     sess->v_residual[li],
            };
            for (size_t i = 0; i < sizeof kv_bufs / sizeof kv_bufs[0]; i++) {
                if (kv_bufs[i] != nullptr) { be->desc->vtbl->buffer_destroy(be, kv_bufs[i]); }
            }
        }
        ...
        struct geist_buffer *infra[] = {
            sess->scratch_normed, sess->scratch_q, sess->scratch_k, sess->scratch_v,
            sess->scratch_attn, sess->scratch_o, sess->scratch_post_attn,
            sess->scratch_h_post_attn, sess->scratch_pre_ff, sess->scratch_gate,
            sess->scratch_up, sess->scratch_ffn_out, sess->scratch_post_ff,
            sess->scratch_h_post_ff, sess->scratch_gate_ple, sess->scratch_proj_ple,
            sess->scratch_ones_headdim_max, sess->scratch_ple_lookup,
            sess->scratch_h_a, sess->scratch_h_b, sess->scratch_per_layer_input,
            sess->scratch_logits,
        };
        for (size_t i = 0; i < sizeof infra / sizeof infra[0]; i++) {
            if (infra[i] != nullptr) { be->desc->vtbl->buffer_destroy(be, infra[i]); }
        }
    }
