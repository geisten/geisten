...
static inline struct geist_tensor view_1d(struct geist_buffer *b, int64_t n) {
    struct geist_tensor t = {
        ...
        .ndim = 1, .shape = {n, 0, 0, 0, 0, 0, 0, 0},
        .stride = {1, 0, 0, 0, 0, 0, 0, 0},
    };
    return t;
}

static inline struct geist_tensor view_2d_at(struct geist_buffer *b,
                                              size_t off_bytes,
                                              int64_t s0, int64_t s1) {
    struct geist_tensor t = {
        .buffer = b, .offset = off_bytes,
        .dtype = GEIST_DTYPE_F32, .layout = GEIST_LAYOUT_DENSE,
        .ndim = 2, .shape = {s0, s1, 0, 0, 0, 0, 0, 0},
        .stride = {s1, 1, 0, 0, 0, 0, 0, 0},
    };
    return t;
}

static inline struct geist_tensor view_2d(struct geist_buffer *b,
                                           int64_t s0, int64_t s1) {
 ...
 */
static inline void apply_per_channel_inv_scale_inplace(
    const struct geist_backend_vtbl *v, struct geist_buffer *buf,
    size_t seq, size_t n, const float *scale) {
    if (scale == nullptr) return;
    float *p = (float *) v->buffer_map(buf);
    for (size_t t = 0; t < seq; t++) {
        float *row = p + t * n;
        for (size_t j = 0; j < n; j++) row[j] *= scale[j];
    }
    v->buffer_unmap(buf);
}

static inline void apply_bitnet_input_quant_inplace(
    const struct geist_backend_vtbl *v, struct geist_buffer *buf,
    size_t seq, size_t n) {
    float *p = (float *) v->buffer_map(buf);
    for (size_t t = 0; t < seq; t++) {
...
/* forward/kv_store.c */
[[nodiscard]] enum geist_status transformer_kv_store_append(
    struct transformer_layer_forward_ctx *ctx);
[[nodiscard]] enum geist_status transformer_kv_store_attention(
    struct transformer_layer_forward_ctx *ctx,
    const struct geist_tensor *t_q_3d,
    struct geist_tensor *t_attn_3d);

/* forward/layer_ffn.c */
[[nodiscard]] enum geist_status transformer_layer_run_ffn_block(
    struct transformer_layer_forward_ctx *ctx);
...
[[nodiscard]] enum geist_status linear_w_no_host_fallback(
    ...
    const struct geist_tensor *t_x, const struct geist_tensor *t_w,
    struct geist_tensor *t_y);
[[nodiscard]] enum geist_status linear_w_scaled_input_or_legacy(
    struct geist_backend *be,
    const struct geist_backend_vtbl *v,
    struct geist_buffer *x_buf, struct geist_buffer *y_buf,
    const struct geist_weight *w,
