...
enum geist_status transformer_kv_store_append(
    ...
    if (ctx->kv_kivi_enabled) {
        ...
        for (size_t t = 0; t < seq; t++) {
            ...
            memcpy(k_res + res_idx * row_elems, k_src + t * row_elems,
                   row_elems * sizeof(float));
            memcpy(v_res + res_idx * row_elems, v_src + t * row_elems,
                   row_elems * sizeof(float));
        }
        ...
        for (size_t t = 0; t < seq; t++) {
            ...
            for (size_t h = 0; h < st->n_kv_heads; h++) {
                ...
                float k_scale = k_amax / 127.0f; if (k_scale == 0.0f) { k_scale = 1.0f; }
                float v_scale = v_amax / 127.0f; if (v_scale == 0.0f) { v_scale = 1.0f; }
...
enum geist_status transformer_kv_store_attention(
    struct transformer_layer_forward_ctx *ctx,
    ...
    if (ctx->kv_kivi_enabled) {
        ...
        if (scores == nullptr) {
            geist_backend_set_error(be, GEIST_E_OOM,
                                    ...
                                    "KIVI scores (kv_len_now=%zu)", kv_len_now);
            return GEIST_E_OOM;
        }
        attention_kivi_via_buffers(qp, ctx->seq, st->n_q_heads, ctx->hd,
                                   kqp, kscp, kzep,
                                   vqp, vscp, vzep,
                                   krp, vrp,
                                   kv_len_now, st->n_kv_heads,
                                   ctx->q_position, L->sliding_window,
                                   st->sess->kivi_drained_count, KIVI_K_GROUP_SIZE,
                                   scores, outp);
        ...
        struct geist_tensor t_kcache_3d = view_3d(ctx->k_cache_buf,
                                                  (int64_t) kv_len_now,
                                                  st->n_kv_heads, (int64_t) ctx->hd);
        struct geist_tensor t_vcache_3d = view_3d(ctx->v_cache_buf,
                                                  (int64_t) kv_len_now,
                                                  st->n_kv_heads, (int64_t) ctx->hd);
        enum geist_status s = v->attention(be, t_q_3d, &t_kcache_3d, &t_vcache_3d,
                                           ctx->q_position, L->sliding_window,
                                           t_attn_3d);
