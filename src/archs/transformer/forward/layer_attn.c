...
static const char * const g_attn_profile_names[ATTN_PROFILE_COUNT] = {
    "norm", "qkv", "q_prep", "kv_prep", "kv_store",
    "core", "post_core", "o_proj", "post",
};
...
static enum geist_status permute_interleaved_rope_inplace(
    ...
    if (x == nullptr) { return GEIST_E_BACKEND; }
...
enum geist_status transformer_layer_run_attention_block(
    struct transformer_layer_forward_ctx *ctx) {
    ...
    if (ctx->apply_bitnet_input_quant) {
        ...
        apply_bitnet_input_quant_inplace(v, st->sess->scratch_normed,
                                          ctx->seq, st->d_model);
        transformer_profile_add(&g_attn_profile, ATTN_PROFILE_NORM, t0);
    }

    struct geist_tensor t_q_2d = view_2d(st->sess->scratch_q,
                                         ctx->SEQ, (int64_t) ctx->q_out);
    t0 = profile ? transformer_profile_now_ns() : 0;
    if (ctx->compute_kv) {
        struct geist_tensor t_k_2d = view_2d(st->sess->scratch_k,
                                             ctx->SEQ, (int64_t) ctx->kv_out);
        struct geist_tensor t_v_2d = view_2d(st->sess->scratch_v,
                                             ctx->SEQ, (int64_t) ctx->kv_out);
        s = linear_w_triple_or_legacy(be, v, st->sess->scratch_normed,
                                      st->sess->scratch_q,
                                      st->sess->scratch_k,
                                      st->sess->scratch_v,
                                      &L->q_proj_w, &L->k_proj_w,
                                      &L->v_proj_w, ctx->seq,
                                      &t_normed_2d,
                                      &L->q_proj, &L->k_proj, &L->v_proj,
                                      &t_q_2d, &t_k_2d, &t_v_2d);
        transformer_profile_add(&g_attn_profile, ATTN_PROFILE_QKV, t0);
        if (s != GEIST_OK) { return s; }
    } else {
        s = linear_w_or_legacy(be, v, st->sess->scratch_normed, st->sess->scratch_q,
                               &L->q_proj_w, ctx->seq,
                               &t_normed_2d, &L->q_proj, &t_q_2d);
        transformer_profile_add(&g_attn_profile, ATTN_PROFILE_QKV, t0);
        if (s != GEIST_OK) { return s; }
    }
    ...
    if (ctx->apply_gemma_attn_norms) {
        struct geist_tensor t_q_perhead = view_2d(st->sess->scratch_q,
                                                  ctx->SEQ * st->n_q_heads,
                                                  (int64_t) ctx->hd);
    ...
    const size_t cos_row_bytes = ctx->hd * sizeof(float);
    struct geist_tensor t_cos = view_2d_at(cos_buf, ctx->q_position * cos_row_bytes,
                                           ctx->SEQ, (int64_t) ctx->hd);
    struct geist_tensor t_sin = view_2d_at(sin_buf, ctx->q_position * cos_row_bytes,
                                           ctx->SEQ, (int64_t) ctx->hd);
    struct geist_tensor t_q_3d = view_3d(st->sess->scratch_q, ctx->SEQ,
                                         st->n_q_heads, (int64_t) ctx->hd);
    if (ctx->rope_interleaved) {
        s = permute_interleaved_rope_inplace(v, st->sess->scratch_q,
                                             ctx->seq, st->n_q_heads, ctx->hd);
        if (s != GEIST_OK) { return s; }
    }
    ...
    if (!ctx->apply_gemma_attn_norms) {
        ...
        for (size_t i = 0; i < ctx->seq * ctx->q_out; i++) { qp[i] *= scale; }
    ...
    if (ctx->compute_kv) {
        t0 = profile ? transformer_profile_now_ns() : 0;
        struct geist_tensor t_k_3d = view_3d(st->sess->scratch_k, ctx->SEQ,
                                             st->n_kv_heads, (int64_t) ctx->hd);
        if (ctx->apply_gemma_attn_norms) {
            struct geist_tensor t_k_perhead = view_2d(st->sess->scratch_k,
                                                      ctx->SEQ * st->n_kv_heads,
                                                      (int64_t) ctx->hd);
            struct geist_tensor t_v_perhead = view_2d(st->sess->scratch_v,
                                                      ctx->SEQ * st->n_kv_heads,
                                                      (int64_t) ctx->hd);
            struct geist_tensor t_k_norm_w = view_1d(L->k_norm.buffer, (int64_t) ctx->hd);
            struct geist_tensor t_ones_hd = view_1d(st->sess->scratch_ones_headdim_max,
                                                    (int64_t) ctx->hd);
        ...
        if (ctx->rope_interleaved) {
            s = permute_interleaved_rope_inplace(v, st->sess->scratch_k,
                                                  ctx->seq, st->n_kv_heads, ctx->hd);
            if (s != GEIST_OK) { return s; }
        }
    ...
    struct geist_tensor t_attn_3d = view_3d(st->sess->scratch_attn, ctx->SEQ,
                                            st->n_q_heads, (int64_t) ctx->hd);
    t0 = profile ? transformer_profile_now_ns() : 0;
    ...
    t0 = profile ? transformer_profile_now_ns() : 0;
    apply_per_channel_inv_scale_inplace(v, st->sess->scratch_attn, ctx->seq,
                                        ctx->q_out, L->o_awq_inv_scale);

    struct geist_tensor t_attn_2d = view_2d(st->sess->scratch_attn,
                                            ctx->SEQ, (int64_t) ctx->q_out);
    if (ctx->apply_sub_ln && L->attn_sub_norm.buffer != nullptr) {
        struct geist_tensor t_attn_sub_w = view_1d(L->attn_sub_norm.buffer,
                                                   (int64_t) ctx->q_out);
    ...
    t0 = profile ? transformer_profile_now_ns() : 0;
    s = linear_w_or_legacy(be, v, st->sess->scratch_attn, st->sess->scratch_o,
                           &L->o_proj_w, ctx->seq, &t_attn_2d, &L->o_proj,
                           &t_o_2d);
    ...
    if (ctx->apply_gemma_attn_norms) {
        struct geist_tensor t_post_attn_2d = view_2d(st->sess->scratch_post_attn,
                                                     ctx->SEQ, st->d_model);
        struct geist_tensor t_w_post_attn = view_1d(L->post_attn_norm.buffer,
                                                    st->d_model);
