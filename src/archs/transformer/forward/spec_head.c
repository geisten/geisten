...
struct spec_cand { float s; uint32_t idx; };
...
static inline int32_t spec_i8dot(const int8_t *a, const int8_t *b, size_t n) {
    ...
    for (; k < n; k++) { s += (int32_t) a[k] * (int32_t) b[k]; }
    return s;
}
...
static bool spec_dtype_ok(uint16_t dt) {
    switch (dt) {
    case GEIST_DTYPE_F16: case GEIST_DTYPE_BF16: case GEIST_DTYPE_F32:
    case GEIST_DTYPE_Q3_K: case GEIST_DTYPE_Q4_K: case GEIST_DTYPE_Q5_K:
    case GEIST_DTYPE_Q6_K: case GEIST_DTYPE_Q8_0:
        return true;
    default:
        return false;
    }
...
static size_t spec_row_stride(uint16_t dt, size_t n) {
    switch (dt) {
    case GEIST_DTYPE_F16: case GEIST_DTYPE_BF16: return n * 2;
    case GEIST_DTYPE_F32: return n * 4;
    case GEIST_DTYPE_Q3_K: return n / Q3_K_BLOCK_ELEMS * Q3_K_BLOCK_BYTES;
    case GEIST_DTYPE_Q4_K: return n / Q4_K_BLOCK_ELEMS * Q4_K_BLOCK_BYTES;
    case GEIST_DTYPE_Q5_K: return n / Q5_K_BLOCK_ELEMS * Q5_K_BLOCK_BYTES;
    case GEIST_DTYPE_Q6_K: return n / Q6_K_BLOCK_ELEMS * Q6_K_BLOCK_BYTES;
    case GEIST_DTYPE_Q8_0: return n / Q8_0_BLOCK_ELEMS * Q8_0_BLOCK_BYTES;
    default: return 0;
    }
}
...
static void spec_row_to_f32(uint16_t dt, const uint8_t *row, size_t n, float *dst) {
    ...
    case GEIST_DTYPE_F16: {
        ...
        for (size_t i = 0; i < n; i++) { dst[i] = (float) w[i]; }
        break;
    }
    ...
    case GEIST_DTYPE_F32: memcpy(dst, row, n * sizeof(float)); break;
    case GEIST_DTYPE_Q3_K: dequant_q3_K_row(row, dst, n); break;
    case GEIST_DTYPE_Q4_K: dequant_q4_K_row(row, dst, n); break;
    case GEIST_DTYPE_Q5_K: dequant_q5_K_row(row, dst, n); break;
    case GEIST_DTYPE_Q6_K: dequant_q6_K_row(row, dst, n); break;
    case GEIST_DTYPE_Q8_0: dequant_q8_0_row(row, dst, n); break;
    default: memset(dst, 0, n * sizeof(float)); break;
    }
}
...
static bool spec_head_build(struct transformer_arch_state *st) {
    ...
    if (w->raw == nullptr || !spec_dtype_ok(w->dtype)) { return false; }
    const size_t H = (size_t) w->n_in;
    const size_t V = (size_t) w->n_out;
    if (H != (size_t) st->d_model || V != (size_t) st->vocab_size) { return false; }
    ...
    while (sub > 1 && H % sub != 0) { sub--; }   /* require H % sub == 0 */
    ...
    if (sketch == nullptr || rscale == nullptr || x_i8 == nullptr ||
        a_sk == nullptr || rough == nullptr || row32 == nullptr ||
        heap == nullptr || scratch == nullptr) {
        safe_free((void **) &sketch); safe_free((void **) &rscale);
        safe_free((void **) &x_i8);   safe_free((void **) &a_sk);
        safe_free((void **) &rough);  safe_free((void **) &row32);
        safe_free((void **) &heap);   safe_free((void **) &scratch);
        return false;
    }
...
bool transformer_spec_head_try(struct transformer_arch_state *st,
    ...
    if (spec_head_env() == 0) { return false; }
    if (st->spec_state == 0) {
        st->spec_state = spec_head_build(st) ? 1 : -1;
    }
    if (st->spec_state != 1) { return false; }
    ...
    if (st->sess->temperature != 0.0f) { return false; }
    ...
    if (h == nullptr || logits == nullptr) {
        if (h != nullptr) { v->buffer_unmap(st->sess->scratch_h_a); }
        if (logits != nullptr) { v->buffer_unmap(st->sess->scratch_logits); }
        return false;
    }
    ...
    for (size_t s = 0; s < SD; s++) { a_sk[s] = x_i8[s * sub]; }
...
#pragma omp parallel for schedule(static)
    ...
    for (size_t r = 0; r < V; r++) {
        ...
        if (hn < topk) {
            /* sift up */
            size_t i = hn++;
            heap[i].s = sc; heap[i].idx = (uint32_t) r;
            while (i > 0) {
                size_t p = (i - 1) >> 1;
                if (heap[p].s <= heap[i].s) { break; }
                struct spec_cand t = heap[p]; heap[p] = heap[i]; heap[i] = t;
                i = p;
            }
        } else if (sc > heap[0].s) {
            heap[0].s = sc; heap[0].idx = (uint32_t) r;
            /* sift down */
            size_t i = 0;
            for (;;) {
                size_t l = 2 * i + 1, rgt = 2 * i + 2, sm = i;
                if (l < topk && heap[l].s < heap[sm].s) { sm = l; }
                if (rgt < topk && heap[rgt].s < heap[sm].s) { sm = rgt; }
                if (sm == i) { break; }
                struct spec_cand t = heap[sm]; heap[sm] = heap[i]; heap[i] = t;
                i = sm;
            }
        }
    }
    ...
    const bool exact_kernel =
        dt != GEIST_DTYPE_F16 &&
        st->embed_table_w.linear_m1 != nullptr &&
        st->embed_table_w.backend_layout == GEIST_W_LAYOUT_SOURCE;
    for (size_t i = 0; i < V; i++) { logits[i] = -INFINITY; }
