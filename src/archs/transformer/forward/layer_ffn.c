...
static const char * const g_ffn_profile_names[FFN_PROFILE_COUNT] = {
    "norm", "gate_up", "act", "mul", "sub_norm",
    "down_scale", "down", "post",
};
...
enum geist_status transformer_layer_run_ffn_block(
    ...
    if (ctx->seq > 1 &&
        ...
        if (xp == nullptr || yp == nullptr) {
            if (xp != nullptr) v->buffer_unmap(st->sess->scratch_pre_ff);
            if (yp != nullptr) v->buffer_unmap(st->sess->scratch_ffn_out);
            return GEIST_E_BACKEND;
        }
        t0 = profile ? transformer_profile_now_ns() : 0;
        s = v->ffn_geglu_q4q6_mN(be, xp, ctx->seq, (size_t) st->d_model,
                                 ctx->inter, &L->gate_proj_w, &L->up_proj_w,
                                 &L->down_proj_w, L->down_awq_inv_scale, yp);
        ...
        if (s != GEIST_E_UNSUPPORTED) { return s; }
    }
    ...
    if (ctx->ffn_activation == GEIST_FFN_SQUARED_RELU) {
        t0 = profile ? transformer_profile_now_ns() : 0;
        s = linear_w_or_legacy(be, v, st->sess->scratch_pre_ff, st->sess->scratch_up,
                               &L->up_proj_w, ctx->seq,
        ...
        struct geist_tensor t_gate_2d = view_2d(st->sess->scratch_gate,
                                                ctx->SEQ, (int64_t) ctx->inter);
        t0 = profile ? transformer_profile_now_ns() : 0;
        s = linear_w_pair_or_legacy(be, v, st->sess->scratch_pre_ff,
                                    st->sess->scratch_gate,
                                    st->sess->scratch_up,
                                    &L->gate_proj_w, &L->up_proj_w,
                                    ctx->seq,
                                    &t_pre_ff_2d, &L->gate_proj, &L->up_proj,
                                    &t_gate_2d, &t_up_2d);
        transformer_profile_add(&g_ffn_profile, FFN_PROFILE_GATE_UP, t0);
        if (s != GEIST_OK) { return s; }
        ...
        if (ctx->ffn_activation == GEIST_FFN_GATED_SQUARED_RELU) {
            ...
            s = v->mul(be, &t_gate_2d, &t_up_2d, &t_gate_2d);
            transformer_profile_add(&g_ffn_profile, FFN_PROFILE_MUL, t0);
        } else if (v->gelu_tanh_mul_scaled != nullptr &&
                   L->down_awq_inv_scale != nullptr &&
                   !has_ffn_sub_norm &&
                   ffn_fused_scale_enabled()) {
            t0 = profile ? transformer_profile_now_ns() : 0;
            s = v->gelu_tanh_mul_scaled(be, &t_gate_2d, &t_up_2d,
                                        L->down_awq_inv_scale, &t_gate_2d);
            transformer_profile_add(&g_ffn_profile, FFN_PROFILE_ACT, t0);
    ...
    if (has_ffn_sub_norm) {
        struct geist_tensor t_ffn_sub_w = view_1d(L->ffn_sub_norm.buffer,
                                                  (int64_t) ctx->inter);
        t0 = profile ? transformer_profile_now_ns() : 0;
    ...
    if (st->runtime_flags.dump_act_sparsity) {
        if (!mid_already_down_scaled) {
            t0 = profile ? transformer_profile_now_ns() : 0;
            apply_per_channel_inv_scale_inplace(v, mid_buf, ctx->seq, ctx->inter,
                                                L->down_awq_inv_scale);
            transformer_profile_add(&g_ffn_profile, FFN_PROFILE_DOWN_SCALE, t0);
        }
        transformer_probe_ffn_sparsity(v, true, ctx->layer_idx, mid_buf,
                                       ctx->seq * ctx->inter);
        t0 = profile ? transformer_profile_now_ns() : 0;
        s = linear_w_or_legacy(be, v, mid_buf, st->sess->scratch_ffn_out,
                               &L->down_proj_w, ctx->seq,
                               &t_mid_2d, &L->down_proj, &t_ffn_out_2d);
        ...
        if (profile) {
            ...
            s = linear_w_scaled_input_or_legacy(be, v, mid_buf,
                                                st->sess->scratch_ffn_out,
                                                &L->down_proj_w, ctx->seq,
                                                ctx->inter, L->down_awq_inv_scale,
                                                &t_mid_2d, &L->down_proj,
                                                &t_ffn_out_2d);
        }
    }
    if (s != GEIST_OK) { return s; }
...
ffn_post:
    ...
    if (ctx->apply_gemma_attn_norms) {
        struct geist_tensor t_post_ff_2d = view_2d(st->sess->scratch_post_ff,
                                                   ctx->SEQ, st->d_model);
        struct geist_tensor t_w_post_ffw = view_1d(L->post_ffw_norm.buffer,
                                                   st->d_model);
