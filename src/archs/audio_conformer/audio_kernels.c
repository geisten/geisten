...
void conv2d_fp32(const float* in, const float* w, float* out,
                  int c_in, int c_out, int h_in, int w_in,
                  int kh, int kw, int sh, int sw, int ph, int pw)
{
    conv2d_fp32_from(in, w, out, c_in, c_out, h_in, w_in,
                      kh, kw, sh, sw, ph, pw, 0);
}
...
void conv2d_fp32_from(const float* in, const float* w, float* out,
                       int c_in, int c_out, int h_in, int w_in,
                       int kh, int kw, int sh, int sw, int ph, int pw,
                       int start_h)
{
...
void layernorm_fp32_ws(const float* x, const float* gamma,
                        size_t batch, size_t d, float eps, float* y)
{
    ...
    for (size_t b = 0; b < batch; b++) {
        ...
        float mean = 0.0f;
        for (size_t i = 0; i < d; i++) mean += xb[i];
        mean *= inv_d;
...
void softplus_fp32(float* x, size_t n) {
    for (size_t i = 0; i < n; i++) x[i] = log1pf(expf(x[i]));
}
...
void depthwise_conv1d_causal_fp32(const float* in, const float* w, float* out,
                                    int channels, int t_in, int kernel)
{
    ...
    for (int c = 0; c < channels; c++) {
        ...
        for (int t = 0; t < t_in; t++) {
            ...
            for (int k = 0; k < kernel; k++) {
                ...
                if (src >= 0 && src < t_in) acc += w_c[k] * in_c[src];
            }
            out_c[t] = acc;
        }
    }
...
void softmax_fp32(float* x, size_t n_rows, size_t d) {
    ...
    for (size_t r = 0; r < n_rows; r++) {
...
#if defined(__ARM_NEON)
        {
            ...
            for (; i + 4 <= d; i += 4) vm = vmaxq_f32(vm, vld1q_f32(row + i));
            m = vmaxvq_f32(vm);
            for (; i < d; i++) if (row[i] > m) m = row[i];
        }
#else
        for (size_t i = 1; i < d; i++) if (row[i] > m) m = row[i];
#endif
        ...
        if (fast_expf) {
            ...
            for (size_t i = 0; i < d; i++) row[i] -= m;
            vvexpf(row, row, &n_int);
            for (size_t i = 0; i < d; i++) sum += row[i];
        } else
#endif
        {
#if defined(__ARM_NEON)
...
#else
        for (size_t i = 0; i < d; i++) row[i] *= inv;
#endif
    }
}
