...
struct ae_stage_timer { double sum_ms; uint64_t count; };
static struct ae_stage_timer g_ae_subsample, g_ae_pos_emb, g_ae_layer_total,
                              g_ae_ffn1, g_ae_norm_pre_attn, g_ae_attn,
                              g_ae_norm_post_attn, g_ae_lconv, g_ae_ffn2,
                              g_ae_norm_out, g_ae_output_proj, g_ae_embed_proj;
static double ae_now_ms_(void) {
    struct timeval tv; gettimeofday(&tv, nullptr);
    return (double)tv.tv_sec * 1000.0 + (double)tv.tv_usec / 1000.0;
}
#define AE_TIC()       const double _ae_t0 = ae_now_ms_()
#define AE_TOC(stg)    do { (stg).sum_ms += ae_now_ms_() - _ae_t0; (stg).count++; } while (0)
static void ae_profile_print_and_reset(void) {
    struct ae_stage_timer* s[] = {
        &g_ae_subsample, &g_ae_pos_emb, &g_ae_layer_total,
        &g_ae_ffn1, &g_ae_norm_pre_attn, &g_ae_attn, &g_ae_norm_post_attn,
        &g_ae_lconv, &g_ae_ffn2, &g_ae_norm_out,
        &g_ae_output_proj, &g_ae_embed_proj,
    };
    const char* names[] = {
        "subsample", "pos_emb+mask", "all_layers_TOTAL",
        "  ffn1", "  norm_pre_attn", "  attn", "  norm_post_attn",
        "  lconv", "  ffn2", "  norm_out",
        "output_proj", "embed_audio_proj",
    };
    double total = 0;
    for (size_t i = 0; i < sizeof s / sizeof s[0]; i++) total += s[i]->sum_ms;
    fprintf(stderr, "  audio-encoder profile (one segment):\n");
    for (size_t i = 0; i < sizeof s / sizeof s[0]; i++) {
        double pct = total > 0 ? 100.0 * s[i]->sum_ms / total : 0.0;
        fprintf(stderr, "    %-22s %8.2f ms  %5.1f%%  (%llu calls)\n",
                names[i], s[i]->sum_ms, pct, (unsigned long long)s[i]->count);
    }
    fprintf(stderr, "    %-22s %8.2f ms\n", "TOTAL (sum)", total);
    for (size_t i = 0; i < sizeof s / sizeof s[0]; i++) {
        s[i]->sum_ms = 0; s[i]->count = 0;
    }
}
#else
...
static float* load_bf16(struct st_ctx* sf, const char* name, size_t expect_elems) {
    ...
    if (!t) { fprintf(stderr, "audio_encoder: missing %s\n", name); return nullptr; }
...
static bool load_clippable(struct st_ctx* sf, const char* prefix, struct ClippableLinear* cl,
                            size_t out_dim, size_t in_dim,
                            enum audio_linear_prec prec) {
    char buf[384];
    snprintf(buf, sizeof(buf), "%slinear.weight", prefix);
    cl->w = load_bf16(sf, buf, out_dim * in_dim);
    if (!cl->w) return false;
    snprintf(buf, sizeof(buf), "%sinput_min", prefix);
    if (!load_bf16_scalar(sf, buf, &cl->input_min)) return false;
    snprintf(buf, sizeof(buf), "%sinput_max", prefix);
    if (!load_bf16_scalar(sf, buf, &cl->input_max)) return false;
    snprintf(buf, sizeof(buf), "%soutput_min", prefix);
    if (!load_bf16_scalar(sf, buf, &cl->output_min)) return false;
    snprintf(buf, sizeof(buf), "%soutput_max", prefix);
    if (!load_bf16_scalar(sf, buf, &cl->output_max)) return false;
    ...
    if (getenv("GEIST_AUDIO_FORCE_QUANT") == nullptr) prec = AUDIO_PREC_FP32;
#endif
...
static bool load_ffn(struct st_ctx* sf, int layer_idx,
                      const char* prefix, struct FFN* ffn) {
    char buf[384];
    ...
    const enum audio_linear_prec ffn_prec =
        layer_active ? AUDIO_PREC_W8A8 : AUDIO_PREC_FP32;
    snprintf(buf, sizeof(buf), "%sffw_layer_1.", prefix);
    if (!load_clippable(sf, buf, &ffn->ffw1, FFN_INTER, AUDIO_HIDDEN, ffn_prec)) return false;
    snprintf(buf, sizeof(buf), "%sffw_layer_2.", prefix);
    if (!load_clippable(sf, buf, &ffn->ffw2, AUDIO_HIDDEN, FFN_INTER, ffn_prec)) return false;
...
static int w8a8_layer_limit(void) {
    ...
    if (limit < 0) {
        ...
        if (limit < 0) limit = 0;
        if (limit > N_LAYERS) limit = N_LAYERS;
    }
    return limit;
}

static bool load_attn(struct st_ctx* sf, int layer_idx,
                       const char* prefix, struct Attn* a) {
    char buf[384];
    ...
    if (!load_clippable(sf, buf, &a->q_proj, AUDIO_HIDDEN, AUDIO_HIDDEN, attn_prec)) return false;
    snprintf(buf, sizeof(buf), "%sk_proj.", prefix);
    if (!load_clippable(sf, buf, &a->k_proj, AUDIO_HIDDEN, AUDIO_HIDDEN, attn_prec)) return false;
    snprintf(buf, sizeof(buf), "%sv_proj.", prefix);
    if (!load_clippable(sf, buf, &a->v_proj, AUDIO_HIDDEN, AUDIO_HIDDEN, attn_prec)) return false;
    snprintf(buf, sizeof(buf), "%spost.", prefix);
    if (!load_clippable(sf, buf, &a->post, AUDIO_HIDDEN, AUDIO_HIDDEN, attn_prec)) return false;
...
static bool load_lconv(struct st_ctx* sf, int layer_idx,
                        const char* prefix, struct LConv* lc) {
    char buf[384];
    ...
    if (!load_clippable(sf, buf, &lc->linear_start, 2 * AUDIO_HIDDEN, AUDIO_HIDDEN, prec)) return false;
    snprintf(buf, sizeof(buf), "%slinear_end.", prefix);
    if (!load_clippable(sf, buf, &lc->linear_end, AUDIO_HIDDEN, AUDIO_HIDDEN, prec)) return false;
...
static bool load_layer(struct st_ctx* sf, int layer_idx, struct ConformerLayer* L) {
    ...
    char sub[384];
    snprintf(sub, sizeof(sub), "%sfeed_forward1.", p);  if (!load_ffn(sf, layer_idx, sub, &L->ff1)) return false;
    snprintf(sub, sizeof(sub), "%sfeed_forward2.", p);  if (!load_ffn(sf, layer_idx, sub, &L->ff2)) return false;
    snprintf(sub, sizeof(sub), "%sself_attn.",     p);  if (!load_attn(sf, layer_idx, sub, &L->attn)) return false;
    snprintf(sub, sizeof(sub), "%slconv1d.",       p);  if (!load_lconv(sf, layer_idx, sub, &L->lconv)) return false;
    snprintf(sub, sizeof(sub), "%snorm_pre_attn.weight",  p); L->norm_pre_attn  = load_bf16(sf, sub, AUDIO_HIDDEN);
    snprintf(sub, sizeof(sub), "%snorm_post_attn.weight", p); L->norm_post_attn = load_bf16(sf, sub, AUDIO_HIDDEN);
    snprintf(sub, sizeof(sub), "%snorm_out.weight",       p); L->norm_out       = load_bf16(sf, sub, AUDIO_HIDDEN);
...
static void quantize_clippable_w8(struct ClippableLinear* cl, size_t out_dim, size_t in_dim) {
    ...
    for (size_t c = 0; c < out_dim; c++) {
        ...
        for (size_t k = 0; k < in_dim; k++) qrow[k] = (int8_t)lrintf(row[k] * inv);
    }
}
...
static void linear_w8a8(const int8_t* w_q8, const float* w_scales,
                         const float* x, float scale_x_inv, float scale_x,
                         size_t m, size_t in_dim, size_t out_dim, float* y) {
    ...
    for (size_t i = 0; i < m; i++) {
        ...
        for (; k < in_dim; k++) qrow[k] = (int8_t)lrintf(xrow[k] * scale_x_inv);
    }
    ...
    for (size_t n = 0; n < out_dim; n++) {
        ...
        for (size_t i = 0; i < m; i++) {
            ...
            for (; k < in_dim; k++) isum += (int32_t)wrow[k] * (int32_t)xrow[k];
...
static void linear_w8a32(const int8_t* w_q8, const float* w_scales,
                          const float* x, size_t m, size_t in_dim, size_t out_dim,
                          float* y) {
#if defined(_OPENMP)
    #pragma omp parallel for schedule(static)
#endif
    for (size_t n = 0; n < out_dim; n++) {
        ...
        for (size_t i = 0; i < m; i++) {
            ...
            for (; k < in_dim; k++) dot += (float)wrow[k] * xrow[k];
...
static void free_clippable(struct ClippableLinear* cl) {
    safe_free((void **) &cl->w); safe_free((void **) &cl->w_q8); safe_free((void **) &cl->w_scales);
    cl->w = nullptr; cl->w_q8 = nullptr; cl->w_scales = nullptr;
}
static void free_ffn(struct FFN* f) { free_clippable(&f->ffw1); free_clippable(&f->ffw2); safe_free((void **) &f->pre_norm); safe_free((void **) &f->post_norm); }
static void free_attn(struct Attn* a) { free_clippable(&a->q_proj); free_clippable(&a->k_proj); free_clippable(&a->v_proj); free_clippable(&a->post); safe_free((void **) &a->relative_k_proj); safe_free((void **) &a->per_dim_scale); }
static void free_lconv(struct LConv* l) { free_clippable(&l->linear_start); free_clippable(&l->linear_end); safe_free((void **) &l->depthwise); safe_free((void **) &l->pre_norm); safe_free((void **) &l->conv_norm); }
static void free_layer(struct ConformerLayer* L) { free_ffn(&L->ff1); free_ffn(&L->ff2); free_attn(&L->attn); free_lconv(&L->lconv); safe_free((void **) &L->norm_pre_attn); safe_free((void **) &L->norm_post_attn); safe_free((void **) &L->norm_out); }

struct AudioEncoder* audio_encoder_create(const char* safetensors_path) {
    const char* err = nullptr;
    struct st_ctx* sf = st_open(safetensors_path, &err);
    if (!sf) { fprintf(stderr, "audio_encoder: %s\n", err); return nullptr; }
...
#define LOAD(field, suf, n) \
    do { snprintf(buf, sizeof(buf), "%s%s", P, suf); \
         a->field = load_bf16(sf, buf, n); \
         if (!a->field) { audio_encoder_destroy(a); return nullptr; } } while (0)
...
#undef LOAD
    ...
    for (int i = 0; i < N_LAYERS; i++) {
        if (!load_layer(sf, i, &a->layers[i])) {
            audio_encoder_destroy(a); return nullptr;
        }
    }
    fprintf(stderr, "audio_encoder: layers loaded.\n");

    /* Final projections. */
    a->output_proj_w   = load_bf16(sf, "model.audio_tower.output_proj.weight",
                                    (size_t)OUTPUT_PROJ_DIMS * AUDIO_HIDDEN);
    a->output_proj_b   = load_bf16(sf, "model.audio_tower.output_proj.bias",
                                    OUTPUT_PROJ_DIMS);
    a->embed_audio_proj = load_bf16(sf, "model.embed_audio.embedding_projection.weight",
...
void audio_encoder_destroy(struct AudioEncoder* a) {
    ...
    if (a->stream) { audio_stream_state_destroy(a->stream); a->stream = nullptr; }
    if (a->mel) mel_destroy(a->mel);
    safe_free((void **) &a->embed_audio_proj);
    safe_free((void **) &a->output_proj_b);
    safe_free((void **) &a->output_proj_w);
    for (int i = 0; i < N_LAYERS; i++) free_layer(&a->layers[i]);
    safe_free((void **) &a->in_proj);
    safe_free((void **) &a->l1_norm); safe_free((void **) &a->l1_conv);
    safe_free((void **) &a->l0_norm); safe_free((void **) &a->l0_conv);
    if (a->sf) st_close(a->sf);
    safe_free((void **) &a);
}
...
int audio_encoder_push_pcm(struct AudioEncoder* a, const int16_t* samples, size_t n) {
    ...
    if (a->mel == nullptr) (void) ensure_mel(a);  /* lazy first time */
    if (a->mel != nullptr) {
        float frame_pcm[MEL_FRAME_LENGTH];
        while (a->mel_n_computed < a->mel_cap &&
               (a->mel_n_computed + 1) * 160 <= a->pcm_len) {
            const size_t k = a->mel_n_computed;
            for (size_t i = 0; i < MEL_FRAME_LENGTH; i++) {
                const long pi = (long) (k * 160 + i) - 160; /* unshift by left-pad */
                frame_pcm[i] = (pi < 0)
                                   ? 0.0f
                                   : (float) a->pcm_buf[pi] / 32768.0f;
            }
            mel_frame_compute(a->mel, frame_pcm,
                              a->mel_buf + k * MEL_N_MEL);
            a->mel_n_computed++;
        }
    }
    ...
    if (a->stream_enabled && a->worker_active &&
        a->mel_n_computed >= a->worker_last_mel + 48) {
        a->worker_kick = true;
...
static int compute_segment_locked(struct AudioEncoder* a) {
    ...
    for (size_t i = 0; i < n_frames; i++) mask[i] = true;
    pthread_mutex_unlock(&a->mtx);
...
size_t audio_encoder_pull_softtokens(struct AudioEncoder* a, float* out, size_t max_out,
                                      int timeout_ms)
{
    pthread_mutex_lock(&a->mtx);
    while (true) {
        if (a->shutdown_flag) { pthread_mutex_unlock(&a->mtx); return 0; }
        ...
        if (a->stream_enabled && a->worker_active && a->stream != nullptr) {
            ...
            if (a->computed_flag) {
                pthread_mutex_unlock(&a->mtx); return 0;
            }
            ...
            if (a->computed_flag && a->n_emitted < a->n_soft) {
                ...
                memcpy(out, a->soft_tokens + a->n_emitted * AUDIO_SOFT_TOKEN_DIM,
                       take * AUDIO_SOFT_TOKEN_DIM * sizeof(float));
                a->n_emitted += take;
                pthread_mutex_unlock(&a->mtx);
                return take;
        ...
        if (timeout_ms == 0) { pthread_mutex_unlock(&a->mtx); return 0; }
        if (timeout_ms < 0) {
            pthread_cond_wait(&a->cv, &a->mtx);
        } else {
            struct timespec dl; make_deadline(&dl, timeout_ms);
            int rc = pthread_cond_timedwait(&a->cv, &a->mtx, &dl);
            if (rc == ETIMEDOUT) { pthread_mutex_unlock(&a->mtx); return 0; }
        }
    }
}
...
void audio_encoder_reset(struct AudioEncoder* a) {
    ...
    a->n_emitted = 0;
    a->n_soft = 0;
    safe_free((void **) &a->soft_tokens); a->soft_tokens = nullptr;
    if (a->stream) audio_stream_state_reset(a->stream);
...
static void clip_linear_apply(const struct ClippableLinear* cl, float* x, size_t n,
                                size_t in_dim, size_t out_dim, float* y) {
    ...
    case AUDIO_PREC_W8A8: {
        ...
        linear_w8a8(cl->w_q8, cl->w_scales, x, 1.0f / scale_x, scale_x,
                    n, in_dim, out_dim, y);
        break;
    }
...
static void ffn_run(const struct FFN* ffn, float* h, size_t n) {
    ...
    for (size_t i = 0; i < hsize; i++) h[i] = h[i] * 0.5f + residual[i];
...
static inline void axpy_head_fp32(float* out, float w, const float* v) {
    ...
    for (int d = 0; d < HEAD_DIM; d += 16) {
        ...
        vst1q_f32(out + d + 0,  o0); vst1q_f32(out + d + 4,  o1);
        vst1q_f32(out + d + 8,  o2); vst1q_f32(out + d + 12, o3);
    }
}
static inline void zero_head_fp32(float* out) {
    const float32x4_t z = vdupq_n_f32(0.0f);
    for (int d = 0; d < HEAD_DIM; d += 16) {
        vst1q_f32(out + d + 0,  z); vst1q_f32(out + d + 4,  z);
        vst1q_f32(out + d + 8,  z); vst1q_f32(out + d + 12, z);
    }
}
#else
static inline float dot_head_fp32(const float* a, const float* b) {
    float acc = 0.0f;
    for (int d = 0; d < HEAD_DIM; d++) acc += a[d] * b[d];
    return acc;
}
static inline void axpy_head_fp32(float* out, float w, const float* v) {
...
static void attn_run(const struct Attn* attn, const float* h, size_t n,
                      const float* pos_emb, const bool* attn_mask, float* y)
{
...
#else
            for (int d = 0; d < HEAD_DIM; d++) qrow[d] *= q_pds[d];
#endif
        }
    }
#if defined(__ARM_NEON)
    {
        ...
        for (; i < total; i++) k[i] *= k_scale;
    }
#else
    for (size_t i = 0; i < n * AUDIO_HIDDEN; i++) k[i] *= k_scale;
#endif
    ...
    for (int b = 0; b < num_blocks; b++) {
        for (int c = 0; c < CONTEXT_SIZE; c++) {
            ...
            if (src < 0 || src >= (int)n) continue;
            memcpy(k_ctx + ((size_t)b * CONTEXT_SIZE + c) * hd_per_t,
                   k + (size_t)src * hd_per_t, hd_per_t * sizeof(float));
            memcpy(v_ctx + ((size_t)b * CONTEXT_SIZE + c) * hd_per_t,
                   v + (size_t)src * hd_per_t, hd_per_t * sizeof(float));
        }
    }
    safe_free((void **) &k); safe_free((void **) &v);
    ...
    linear_fp32(pos_emb, attn->relative_k_proj, nullptr,
                POS_LEN, AUDIO_HIDDEN, AUDIO_HIDDEN, rel_k);
        ...
        for (int hd = 0; hd < N_HEADS; hd++) {
            ...
            for (int i = 0; i < CHUNK_SIZE; i++) {
                for (int j = 0; j < CONTEXT_SIZE; j++) {
                    ...
                    if (attn_mask &&
                        !attn_mask[((size_t)b * CHUNK_SIZE + i) * CONTEXT_SIZE + j]) {
                        s = -1e9f;
                    }
    ...
    safe_free((void **) &rel_k); safe_free((void **) &k_ctx); safe_free((void **) &v_ctx); safe_free((void **) &q);
...
/* LightConv1D: norm → linear_start (×2) → GLU → causal depthwise conv → conv_norm → SiLU → linear_end → +residual */
static void lconv_run(const struct LConv* lc, float* h, size_t n) {
    ...
    depthwise_conv1d_causal_fp32(hct, lc->depthwise, conv_out,
                                  AUDIO_HIDDEN, (int)n, CONV_KERNEL);
    safe_free((void **) &hct);
    ...
    for (size_t i = 0; i < hsize; i++) h[i] += residual[i];
    safe_free((void **) &residual);
}

void audio_encoder_layer_run(const struct AudioEncoder* a, int layer_idx,
                              const float* h_in, size_t n,
                              const float* pos_emb,
                              ...
                              float* h_out)
{
    ...
    { AE_TIC(); ffn_run(&L->ff1, h, n); AE_TOC(g_ae_ffn1); }

    /* 2. residual + norm_pre_attn + self_attn + norm_post_attn + residual */
    float* residual = heap_alloc_array_aligned(float, hsize);
    memcpy(residual, h, hsize * sizeof(float));
    { AE_TIC(); rmsnorm_fp32(h, L->norm_pre_attn, n, AUDIO_HIDDEN, RMS_EPS, h); AE_TOC(g_ae_norm_pre_attn); }

    float* attn_out = heap_alloc_array_aligned(float, hsize);
    { AE_TIC(); attn_run(&L->attn, h, n, pos_emb, attn_mask_5d, attn_out); AE_TOC(g_ae_attn); }
    memcpy(h, attn_out, hsize * sizeof(float));
    safe_free((void **) &attn_out);

    { AE_TIC(); rmsnorm_fp32(h, L->norm_post_attn, n, AUDIO_HIDDEN, RMS_EPS, h); AE_TOC(g_ae_norm_post_attn); }
    for (size_t i = 0; i < hsize; i++) h[i] += residual[i];
    safe_free((void **) &residual);

    /* 3. lconv1d (internal residual) */
    { AE_TIC(); lconv_run(&L->lconv, h, n); AE_TOC(g_ae_lconv); }

    /* 4. feed_forward2 (internal residual) */
    { AE_TIC(); ffn_run(&L->ff2, h, n); AE_TOC(g_ae_ffn2); }

    /* 5. norm_out */
    { AE_TIC(); rmsnorm_fp32(h, L->norm_out, n, AUDIO_HIDDEN, RMS_EPS, h_out); AE_TOC(g_ae_norm_out); }
...
bool* audio_encoder_compute_attn_mask(const struct AudioEncoder* a, size_t n) {
    ...
    for (int b = 0; b < num_blocks; b++) {
        for (int i = 0; i < CHUNK_SIZE; i++) {
            ...
            if (q_g >= (int)n) continue;          /* invalid query row → mask all False */
            for (int c = 0; c < CONTEXT_SIZE; c++) {
                const int k_g = b * CHUNK_SIZE + c - MAX_PAST_HORIZON;
                if (k_g < 0 || k_g >= (int)n) continue;
                ...
                if (off < -(MAX_PAST_HORIZON - 1) || off > MAX_FUTURE) continue;
...
static void subsample_layer_from(
    const float* w_conv, const float* w_norm,
    const float* in, int c_in, int h_in, int w_in,
    int c_out,
    float* out, int* h_out_p, int* w_out_p,
    int start_h)
{
    const int kh = 3, kw = 3, sh = 2, sw = 2, ph = 1, pw = 1;
    const int h_out = (h_in + 2 * ph - kh) / sh + 1;
    const int w_out = (w_in + 2 * pw - kw) / sw + 1;
    *h_out_p = h_out; *w_out_p = w_out;
    if (start_h < 0)      start_h = 0;
    if (start_h >= h_out) return;

    /* 1: conv → (c_out, h_out, w_out), only oh >= start_h written. */
    conv2d_fp32_from(in, w_conv, out, c_in, c_out, h_in, w_in,
                      kh, kw, sh, sw, ph, pw, start_h);
    ...
    for (int c = 0; c < c_out; c++) {
        ...
        for (int i = 0; i < new_h * w_out; i++) {
            if (slot[i] < 0.0f) slot[i] = 0.0f;
        }
    }
}

/* Thin wrapper for the existing call sites — full h_out range. */
static void subsample_layer(
    const float* w_conv, const float* w_norm,
    const float* in, int c_in, int h_in, int w_in,
    int c_out,
    float* out, int* h_out_p, int* w_out_p)
{
    subsample_layer_from(w_conv, w_norm, in, c_in, h_in, w_in, c_out,
                          out, h_out_p, w_out_p, 0);
}
...
static void apply_time_mask(float* buf, const bool* mask, int c, int t, int w) {
    ...
    for (int ti = 0; ti < t; ti++) {
        ...
        for (int ci = 0; ci < c; ci++) {
            ...
            for (int wi = 0; wi < w; wi++) row[wi] = 0.0f;
        }
    }
}

size_t audio_encoder_subsample_run(const struct AudioEncoder* a, const float* mel_in,
                                    const bool* mask_in,
                                    size_t n_mel_frames, float* out)
{
    ...
    if (mask_in) {
        ...
        for (int i = 0; i < hh; i++) mask1[i] = (i * 2 < T) ? mask_in[i * 2] : false;
    ...
    linear_fp32(flat, a->in_proj, nullptr,
                (size_t)T_out1, AUDIO_HIDDEN, AUDIO_HIDDEN, out);
    safe_free((void **) &flat);
 ...
 * new h positions. */
static size_t audio_encoder_subsample_run_inc(
    const struct AudioEncoder* a,
    struct subs_cache* subs,
    const float* mel_in, const bool* mask_in,
    size_t n_mel_total,
    float* out_sub_buf)
{
    if (n_mel_total <= subs->n_mel_seen) return subs->n_t_out1;
    ...
    if ((size_t)T_out0_new > SUBS_T_OUT0_CAP || (size_t)T_out1_new > SUBS_T_OUT1_CAP) {
        ...
        return audio_encoder_subsample_run(a, mel_in, mask_in, n_mel_total,
                                           out_sub_buf);
    }
    ...
    float* l0_tmp = heap_alloc_array_aligned(
        float, (size_t)SUBS_L0_CHANNELS * T_out0_new * W_out0);
    ...
    int hh, ww;
    subsample_layer_from(a->l0_conv, a->l0_norm, in_for_conv0,
                         1, T, W, SUBS_L0_CHANNELS,
                         l0_tmp, &hh, &ww, (int)subs->n_t_out0);
    safe_free((void **) &mel_masked);
    ...
    float* l1_tmp = heap_alloc_array_aligned(
        float, (size_t)SUBS_L1_CHANNELS * T_out1_new * W_out1);
    /* Old l1 rows. */
    ...
    int hh2, ww2;
    subsample_layer_from(a->l1_conv, a->l1_norm, l0_tmp,
                         SUBS_L0_CHANNELS, hh, ww, SUBS_L1_CHANNELS,
                         l1_tmp, &hh2, &ww2, (int)subs->n_t_out1);
    ...
    linear_fp32(flat, a->in_proj, nullptr,
                (size_t)new_t1, AUDIO_HIDDEN, AUDIO_HIDDEN,
...
size_t audio_encoder_run(const struct AudioEncoder* a, const float* mel_in,
                          const bool* mel_mask_in, size_t n_mel_frames,
                          float* softtokens_out)
{
    ...
    for (int li = 0; li < N_LAYERS; li++) {
        ...
        float* tmp = h_a; h_a = h_b; h_b = tmp;   /* result lands in h_a */
    }
    AE_TOC(g_ae_layer_total); }
    safe_free((void **) &h_b);
    safe_free((void **) &attn_mask); safe_free((void **) &pos_emb);

    /* Step 4: output_proj (1024 → 1536, with bias). */
    float* op = heap_alloc_array_aligned(float, n_sub * OUTPUT_PROJ_DIMS);
    { AE_TIC();
      linear_fp32(h_a, a->output_proj_w, a->output_proj_b,
                  n_sub, AUDIO_HIDDEN, OUTPUT_PROJ_DIMS, op);
    ...
    safe_free((void **) &op);
    { AE_TIC();
      linear_fp32(normed, a->embed_audio_proj, nullptr,
                  n_sub, OUTPUT_PROJ_DIMS, TEXT_HIDDEN, softtokens_out);
...
static struct audio_stream_state* audio_stream_state_create(void) {
    ...
    s->subs.l0 = heap_calloc_array_aligned(float,
                    (size_t)SUBS_L0_CHANNELS * SUBS_T_OUT0_CAP * SUBS_W_OUT0);
    s->subs.l1 = heap_calloc_array_aligned(float,
                    (size_t)SUBS_L1_CHANNELS * SUBS_T_OUT1_CAP * SUBS_W_OUT1);
...
static void attn_run_streaming_block(struct audio_stream_state* state,
                                       ...
                                       const float* h_chunk_in, size_t n_valid,
                                       size_t block_idx,
                                       const float* pos_emb,
                                       float* y_chunk_out)
{
    ...
    for (size_t i = 0; i < n_valid * AUDIO_HIDDEN; i++) k_new[i] *= k_scale;
    ...
    safe_free((void **) &k_new); safe_free((void **) &v_new);
    ...
    for (int c = 0; c < CONTEXT_SIZE; c++) {
        ...
        if (src < 0 || src >= (int)kv_n_after) continue;
        memcpy(k_ctx + (size_t)c * hd_per_t, kv->k + (size_t)src * hd_per_t,
               hd_per_t * sizeof(float));
        memcpy(v_ctx + (size_t)c * hd_per_t, kv->v + (size_t)src * hd_per_t,
               hd_per_t * sizeof(float));
    }
    ...
    for (int i = 0; i < (int)n_valid; i++) {
        ...
        if (q_g >= (int)kv_n_after) continue;
        for (int c = 0; c < CONTEXT_SIZE; c++) {
            const int k_g = (int)block_idx * CHUNK_SIZE + c - MAX_PAST_HORIZON;
            if (k_g < 0 || k_g >= (int)kv_n_after) continue;
            const int off = k_g - q_g;
            if (off < -(MAX_PAST_HORIZON - 1) || off > MAX_FUTURE) continue;
    ...
    for (int hd = 0; hd < N_HEADS; hd++) {
        ...
        for (int i = 0; i < CHUNK_SIZE; i++) {
            for (int j = 0; j < CONTEXT_SIZE; j++) {
                float s = scores_ac[i * CONTEXT_SIZE + j] + scores_bd[i * CONTEXT_SIZE + j];
                s = tanhf(s / ATTN_SOFTCAP) * ATTN_SOFTCAP;
                if (!block_mask[i * CONTEXT_SIZE + j]) s = -1e9f;
    ...
    safe_free((void **) &rel_k); safe_free((void **) &k_ctx);
    safe_free((void **) &v_ctx); safe_free((void **) &q);
...
static void lconv_run_streaming(struct audio_stream_state* state, int layer_idx,
                                  const struct LConv* lc, float* h, size_t n)
{
    const size_t hsize = (size_t)n * AUDIO_HIDDEN;
    ...
    if (lcs->n_filled > 0) {
        ...
        memcpy(ext_in + off * AUDIO_HIDDEN, lcs->hist,
    ...
    depthwise_conv1d_causal_fp32(ext_ct, lc->depthwise, conv_out,
                                  AUDIO_HIDDEN, (int)ext_T, CONV_KERNEL);
    ...
    if (n >= new_n_filled) {
        memcpy(lcs->hist, h_glu + (n - new_n_filled) * AUDIO_HIDDEN,
        ...
        memcpy(tmp, lcs->hist + prior_off * AUDIO_HIDDEN,
               take_prior * AUDIO_HIDDEN * sizeof(float));
        memcpy(tmp + take_prior * AUDIO_HIDDEN, h_glu,
               n * AUDIO_HIDDEN * sizeof(float));
                                                ...
                                                const float* pos_emb)
{
    ...
    float* attn_out = heap_alloc_array_aligned(float, hsize);
    attn_run_streaming_block(state, layer_idx, &L->attn, h_io, n_valid,
                              block_idx, pos_emb, attn_out);
    memcpy(h_io, attn_out, hsize * sizeof(float));
    safe_free((void **) &attn_out);

    rmsnorm_fp32(h_io, L->norm_post_attn, n_valid, AUDIO_HIDDEN, RMS_EPS, h_io);
    for (size_t i = 0; i < hsize; i++) h_io[i] += residual[i];
    safe_free((void **) &residual);
...
size_t audio_encoder_stream_push(const struct AudioEncoder* a,
                                   struct audio_stream_state* state,
                                   const float* mel_full, const bool* mel_mask,
                                   size_t n_mel_total, bool is_final)
{
    if (state == nullptr || n_mel_total == 0) return 0;
    ...
    const size_t n_sub_full = subs_inc
        ? audio_encoder_subsample_run_inc(a, &state->subs, mel_full, mel_mask,
                                            n_mel_total, state->sub_buf)
        : audio_encoder_subsample_run(a, mel_full, mel_mask, n_mel_total,
                                       state->sub_buf);

    if (n_sub_full <= state->n_sub_total) return 0;
    ...
    const size_t block_end_excl = is_final
        ? (n_sub_full + CHUNK_SIZE - 1) / CHUNK_SIZE
        : n_sub_full / CHUNK_SIZE;
    if (block_end_excl <= block_start) return 0;
    ...
    for (size_t b = block_start; b < block_end_excl; b++) {
        ...
        const size_t sub_end   = (sub_start + CHUNK_SIZE) < n_sub_full
                                  ? (sub_start + CHUNK_SIZE)
                                  : n_sub_full;
        const size_t n_chunk = sub_end - sub_start;

        memset(h_chunk, 0, sizeof(h_chunk));
        memcpy(h_chunk, state->sub_buf + sub_start * AUDIO_HIDDEN,
               n_chunk * AUDIO_HIDDEN * sizeof(float));

        for (int li = 0; li < N_LAYERS; li++) {
            audio_encoder_layer_run_streaming(a, state, li, h_chunk, n_chunk,
                                                b, pos_emb);
        }

        /* output_proj + embed_audio on the n_chunk real rows. */
        float* op = heap_alloc_array_aligned(float, n_chunk * OUTPUT_PROJ_DIMS);
        linear_fp32(h_chunk, a->output_proj_w, a->output_proj_b,
                    n_chunk, AUDIO_HIDDEN, OUTPUT_PROJ_DIMS, op);
        ...
        linear_fp32(normed, a->embed_audio_proj, nullptr,
                    n_chunk, OUTPUT_PROJ_DIMS, TEXT_HIDDEN,
...
static void worker_do_push(struct AudioEncoder* a, size_t mel_snap, bool is_final) {
    ...
    for (size_t i = 0; i < mel_snap; i++) mask[i] = true;
    (void) audio_encoder_stream_push(a, a->stream, a->mel_buf, mask,
                                       mel_snap, is_final);
    safe_free((void **) &mask);
}
...
static bool stream_worker_start(struct AudioEncoder* a) {
    ...
    a->worker_final   = false;
    a->worker_last_mel = 0;
    if (pthread_create(&a->worker_tid, nullptr,
                        audio_encoder_stream_worker, a) != 0) {
