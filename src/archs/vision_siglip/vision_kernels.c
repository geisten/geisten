...
void avgpool2d_k3_fp32(const float *in, float *out,
    ...
    for (size_t py = 0; py < pool_h; py++) {
        for (size_t px = 0; px < pool_w; px++) {
            ...
            for (size_t k = 0; k < hidden; k++) o[k] *= inv9;
        }
    }
}
...
void rope_2d_split_fp32(float *x,
    ...
    if (!inv_freq || !cos_x || !sin_x || !cos_y || !sin_y) {
        if (inv_freq) safe_free((void **) &inv_freq);
        if (cos_x)    safe_free((void **) &cos_x);
        if (sin_x)    safe_free((void **) &sin_x);
        if (cos_y)    safe_free((void **) &cos_y);
        if (sin_y)    safe_free((void **) &sin_y);
        return;
    }
...
void vision_attention_bidir_fp32(const float *q, const float *k, const float *v,
    ...
    float *scores_pool = heap_alloc_array_aligned(float,
                            (size_t) n_threads * per_thread_scores);
    if (scores_pool == nullptr) return;
    ...
    for (size_t h = 0; h < n_heads; h++) {
        ...
        if (fast_expf) {
            ...
            for (size_t i = 0; i < n_tokens; i++) {
                ...
                for (size_t j = 0; j < n_tokens; j++) row[j] -= maxv;
                vvexpf(row, row, &n_int);
                float sum = 0.0f;
                for (size_t j = 0; j < n_tokens; j++) sum += row[j];
                float inv = 1.0f / sum;
                for (size_t j = 0; j < n_tokens; j++) row[j] *= inv;
            }
        } else
#endif
        {
