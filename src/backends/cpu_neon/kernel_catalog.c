...
cpu_neon_isa_mask cpu_neon_isa_from_probe(const struct geist_hw_probe *hw) {
    ...
    if (hw->has_neon)    { m |= CPU_NEON_ISA_NEON;    }
    if (hw->has_dotprod) { m |= CPU_NEON_ISA_DOTPROD; }
    if (hw->has_fp16)    { m |= CPU_NEON_ISA_FP16;    }
    return m;
}
...
const char *cpu_neon_isa_mask_name(cpu_neon_isa_mask mask) {
    ...
    int n = snprintf(buf, sizeof(buf), "{%s%s%s%s}",
                      ...
                      ((mask & CPU_NEON_ISA_NEON) &&
                       ((mask & CPU_NEON_ISA_DOTPROD) ||
                        (mask & CPU_NEON_ISA_FP16))) ? ","        : "",
                      (mask & CPU_NEON_ISA_DOTPROD) ? "DOTPROD"   : "",
                      (mask & CPU_NEON_ISA_FP16)
                          ? ((mask & CPU_NEON_ISA_DOTPROD) ? ",FP16" : "FP16")
                          : "");
    (void) n;
    return buf;
}
...
struct cpu_neon_kernel_policy cpu_neon_kernel_policy_default(
    ...
    p.q4k_mtile_prefill =
        cpu_neon_env_bool("GEIST_Q4K_MTILE_PREFILL", p.q4k_mtile_prefill);
    p.q4k_ntile_prefill =
        cpu_neon_env_bool("GEIST_Q4K_NTILE_PREFILL", p.q4k_ntile_prefill);
    p.q4k_triple_prefill =
        cpu_neon_env_bool("GEIST_Q4K_TRIPLE_PREFILL", p.q4k_triple_prefill);
    p.q4k_block_q8_prefill =
        cpu_neon_env_bool("GEIST_Q4K_BLOCK_Q8_PREFILL", p.q4k_block_q8_prefill);
    p.q6k_ntile_prefill =
        cpu_neon_env_bool("GEIST_Q6K_NTILE_PREFILL", p.q6k_ntile_prefill);
    p.q6k_ntile8_prefill =
        cpu_neon_env_bool("GEIST_Q6K_NTILE8_PREFILL", p.q6k_ntile8_prefill);
    p.q6k_ntile4_stream_prefill =
        cpu_neon_env_bool("GEIST_Q6K_NTILE4_STREAM_PREFILL",
                          p.q6k_ntile4_stream_prefill);
    p.q4k_sgemm_prefill =
        cpu_neon_env_bool("GEIST_Q4K_SGEMM_PREFILL", p.q4k_sgemm_prefill);
    p.q6k_sgemm_prefill =
        cpu_neon_env_bool("GEIST_Q6K_SGEMM_PREFILL", p.q6k_sgemm_prefill);
...
bool cpu_neon_should_install_tl1(const struct cpu_neon_kernel_policy *policy,
                                 ...
                                 size_t tl1_bytes) {
    if (policy == nullptr || !policy->tq2_0_tl1_m1) { return false; }
