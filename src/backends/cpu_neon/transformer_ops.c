...
static size_t ffn_tile_blocks(void) {
    ...
    if (env != nullptr && env[0] != '\0') {
        ...
        if (end != env && v >= 1 && v <= 8) return (size_t) v;
    }
    return 2;
}

[[nodiscard]] enum geist_status cpu_neon_ffn_geglu_q4q6_mN(
    struct geist_backend *be, const float *x, size_t m, size_t d_model,
    size_t inter, const struct geist_weight *gate, const struct geist_weight *up,
    const struct geist_weight *down, const float *down_scale, float *y) {
    ...
    if (m == 0 || m > GEIST_QUANT_M_CAP ||
        d_model == 0 || inter == 0 ||
        d_model % Q4_K_BLOCK_ELEMS != 0 ||
        inter % Q6_K_BLOCK_ELEMS != 0 ||
        gate->dtype != GEIST_DTYPE_Q4_K ||
        up->dtype != GEIST_DTYPE_Q4_K ||
        down->dtype != GEIST_DTYPE_Q6_K ||
        (size_t) gate->n_in != d_model ||
        (size_t) up->n_in != d_model ||
        (size_t) down->n_in != inter ||
        (size_t) gate->n_out != inter ||
        (size_t) up->n_out != inter ||
...
#pragma omp parallel for schedule(static) if (m >= 4)
    ...
    for (size_t i = 0; i < m; i++) {
        ws->qk_mN_sc[i] = quantize_x_for_q4k(x + i * d_model, d_model,
    ...
    for (size_t b0 = 0; b0 < total_inter_blocks; b0 += tile_blocks_req) {
        const size_t nb = (total_inter_blocks - b0 < tile_blocks_req)
            ? (total_inter_blocks - b0)
            : tile_blocks_req;
        ...
        linear_q4k_w4a8_prefill_pre(ws->qk_mN_xq, ws->qk_mN_sc,
                                     ws->qk_mN_sum32, m, gate_tile,
                                     d_model, tile_n, ws->ffn_gate);
        linear_q4k_w4a8_prefill_pre(ws->qk_mN_xq, ws->qk_mN_sc,
                                     ws->qk_mN_sum32, m, up_tile,
                                     d_model, tile_n, ws->ffn_up);
        ...
        for (size_t i = 0; i < m; i++) {
            ws->ffn_mid_sc[i] = quantize_x_int8_sym(ws->ffn_mid + i * tile_n,
                                                    tile_n,
                                                    ws->ffn_mid_q8 + i * tile_n);
        }

        linear_q6k_w6a8_prefill_pre_accum_blocks(
            ws->ffn_mid_q8, ws->ffn_mid_sc, m, down->raw, inter,
            d_model, b0, nb, y);
    }
    return GEIST_OK;
}
...
[[nodiscard]] enum geist_status cpu_neon_rope_apply(struct geist_backend      *be,
    ...
    if (xp == nullptr || cosp == nullptr || sinp == nullptr) {
        geist_backend_set_error(be, GEIST_E_UNSUPPORTED,
                                "cpu_neon rope_apply: tensors must be F32 DENSE");
        return GEIST_E_UNSUPPORTED;
    }
    ...
    if (cos->shape[0] != (int64_t) seq_len || sin->shape[0] != (int64_t) seq_len) {
        geist_backend_set_error(be, GEIST_E_INVALID_ARG,
                                "cpu_neon rope_apply: cos/sin seq_len mismatch");
        return GEIST_E_INVALID_ARG;
    }
...
[[nodiscard]] enum geist_status cpu_neon_embedding_lookup(struct geist_backend      *be,
    ...
    if (tablep == nullptr || outp == nullptr || embed_table->ndim != 2) {
        geist_backend_set_error(be, GEIST_E_INVALID_ARG,
                                "cpu_neon embedding_lookup: bad inputs");
        return GEIST_E_INVALID_ARG;
    }
...
static inline void zero_f32_neon(float *x, size_t n) {
    ...
    for (; i + 4 <= n; i += 4) { vst1q_f32(x + i, z); }
    for (; i < n; i++) { x[i] = 0.0f; }
}

static inline void add_scaled_f32_neon(float *dst, const float *src,
...
static inline void add_scaled_f32_neon_256(float *dst, const float *src,
                                           float scale) {
    const float32x4_t s = vdupq_n_f32(scale);
    for (size_t i = 0; i < 256; i += 16) {
        float32x4_t d0 = vld1q_f32(dst + i);
...
static void attention_set_omp_schedule(void) {
    ...
    if (env != nullptr && env[0] != '\0') {
        ...
        if (comma != nullptr && comma[1] != '\0') {
            ...
            if (parsed > 0 && parsed < (1L << 20)) { chunk = (int) parsed; }
        }
    }
...
static bool attention_mqa1_causal_kv_neon(
    const float *q, const float *k, const float *v,
    size_t n_q, size_t n_kv, size_t q_offset,
    size_t n_q_heads, size_t head_dim,
    size_t sliding_window, float *out) {
    if (q == nullptr || k == nullptr || v == nullptr || out == nullptr ||
        n_q == 0 || n_kv == 0 || n_q_heads == 0 || head_dim == 0) {
        return false;
    }
    ...
    if (score_arena == nullptr) { return false; }
...
#pragma omp for schedule(runtime)
        for (size_t idx = 0; idx < total; idx++) {
            ...
            const size_t s_lo = (sliding_window > 0 &&
                                 q_pos + 1 > sliding_window)
                                    ? q_pos + 1 - sliding_window
                                    : 0;
            size_t s_hi = q_pos;
            if (s_hi >= n_kv) { s_hi = n_kv - 1; }

            const float *qv = q + (t * n_q_heads + h) * head_dim;
            float max_score = -INFINITY;
            for (size_t s = s_lo; s <= s_hi; s++) {
                const float *kv = k + s * head_dim;
                const float sc = hd256 ? dot_f32_neon_256(qv, kv)
                                       : dot_f32_neon(qv, kv, head_dim);
                scores[s] = sc;
            ...
            if (hd256) {
                ...
                for (size_t s = s_lo; s <= s_hi; s++) {
                    add_scaled_f32_neon_256(outv, v + s * head_dim,
                                            scores[s] * inv_sum);
                }
            } else {
                zero_f32_neon(outv, head_dim);
                for (size_t s = s_lo; s <= s_hi; s++) {
                    add_scaled_f32_neon(outv, v + s * head_dim,
                                        scores[s] * inv_sum, head_dim);
                }
            }
        }
    }
...
static bool attention_mqa_causal_kv_neon(
    const float *q, const float *k, const float *v,
    size_t n_q, size_t n_kv, size_t q_offset,
    size_t n_q_heads, size_t n_kv_heads, size_t head_dim,
    size_t sliding_window, float *out) {
    if (q == nullptr || k == nullptr || v == nullptr || out == nullptr ||
        n_q == 0 || n_kv == 0 || n_q_heads == 0 || n_kv_heads == 0 ||
        head_dim == 0 || n_q_heads % n_kv_heads != 0) {
        return false;
    }
    const size_t total = n_q * n_q_heads;
    if (total < 16) { return false; }
...
[[nodiscard]] enum geist_status cpu_neon_attention(struct geist_backend      *be,
    ...
    if (qp == nullptr || kp == nullptr || vp == nullptr || op == nullptr) {
        geist_backend_set_error(be, GEIST_E_UNSUPPORTED,
                                "cpu_neon attention: tensors must be F32 DENSE");
        return GEIST_E_UNSUPPORTED;
    }
    ...
    if (k->shape[2] != (int64_t) head_dim || v->shape[2] != (int64_t) head_dim ||
        ...
        geist_backend_set_error(be, GEIST_E_INVALID_ARG,
                                "cpu_neon attention: shape mismatch");
        return GEIST_E_INVALID_ARG;
    }
#if defined(__ARM_NEON)
    if (n_kv_heads == 1 &&
        attention_mqa1_causal_kv_neon(qp, kp, vp, n_q, n_kv, q_offset,
                                      n_q_heads, head_dim, sliding_window, op)) {
        return GEIST_OK;
    }
    if (attention_mqa_causal_kv_neon(qp, kp, vp, n_q, n_kv, q_offset,
