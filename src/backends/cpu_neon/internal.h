...
struct cpu_neon_workspace {
    ...
    int8_t  *m1_xq;    size_t m1_xq_cap;
    int32_t *m1_bsum;  size_t m1_bsum_cap;
    /* tq2_0 q8a M>1 path: m × n_in int8 activations, m scales,
     * m × blocks_per_row per-block sums. */
    int8_t  *mN_xq;    size_t mN_xq_cap;
    float   *mN_sc;    size_t mN_sc_cap;
    int32_t *mN_bsum;  size_t mN_bsum_cap;
    /* q4_K/q5_K W*A8 M>1 path: shared activation quant workspace. */
    int8_t  *qk_mN_xq;    size_t qk_mN_xq_cap;
    float   *qk_mN_sc;    size_t qk_mN_sc_cap;
    int32_t *qk_mN_sum32; size_t qk_mN_sum32_cap;
    ...
    float   *dequant_w_fp32;  size_t dequant_w_fp32_cap;
    /* Experimental fused FFN tile path: gate/up/mid float tiles plus
     * quantized mid activations for Q6_K down accumulation. */
    float   *ffn_gate;   size_t ffn_gate_cap;
    float   *ffn_up;     size_t ffn_up_cap;
    float   *ffn_mid;    size_t ffn_mid_cap;
    int8_t  *ffn_mid_q8; size_t ffn_mid_q8_cap;
    float   *ffn_mid_sc; size_t ffn_mid_sc_cap;
    /* F32 elementwise workspace (currently vForce tanh input). */
    float   *elt_f32;     size_t elt_f32_cap;
};
...
const int8_t *iq_flat_cache_lookup(const struct iq_flat_cache *cache,
                                    const void                 *key);
...
void cpu_neon_w_tq2_0_q8a_m1(const float *x, const struct geist_weight *w,
                             struct geist_backend *be, float *y);
void cpu_neon_w_tq2_0_q8a_mN(const float *x, const struct geist_weight *w,
...
[[nodiscard]] enum geist_status cpu_neon_gelu_tanh_mul(
    struct geist_backend *be, const struct geist_tensor *x, const struct geist_tensor *z,
    struct geist_tensor *y);
[[nodiscard]] enum geist_status cpu_neon_gelu_tanh_mul_scaled(
    struct geist_backend *be, const struct geist_tensor *x, const struct geist_tensor *z,
    const float *scale, struct geist_tensor *y);
[[nodiscard]] enum geist_status cpu_neon_relu_squared(
    struct geist_backend *be, const struct geist_tensor *x, struct geist_tensor *y);
[[nodiscard]] enum geist_status cpu_neon_silu(
    struct geist_backend *be, const struct geist_tensor *x, struct geist_tensor *y);
[[nodiscard]] enum geist_status cpu_neon_rmsnorm(
    struct geist_backend *be, const struct geist_tensor *x, const struct geist_tensor *w,
    float eps, struct geist_tensor *y);

/* Transformer ops — wraps gemma4_kernels.c reference kernels. */
[[nodiscard]] enum geist_status cpu_neon_rope_apply(
    struct geist_backend *be, struct geist_tensor *x,
    const struct geist_tensor *cos, const struct geist_tensor *sin);
[[nodiscard]] enum geist_status cpu_neon_embedding_lookup(
    struct geist_backend *be, const struct geist_tensor *embed_table,
    geist_token_t token_id, struct geist_tensor *out);
[[nodiscard]] enum geist_status cpu_neon_attention(
    struct geist_backend *be, const struct geist_tensor *q, const struct geist_tensor *k,
    const struct geist_tensor *v, size_t q_offset, size_t sliding_window,
