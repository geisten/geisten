...
#else
    (void)x_q8; (void)scale_x; (void)w_iq2s; (void)n_in; (void)n_out; (void)y;
...
void linear_iq2s_w2a8_prefill_pre(const int8_t* x_q8, const float* scale_x,
    ...
    for (size_t n = 0; n < n_out; n++) {
        ...
        for (size_t b = 0; b < n_blocks_per_row; b++) {
            ...
            for (size_t i = 0; i < m; i++) row_acc[i] = vdupq_n_s32(0);
    ...
    (void)w_iq2s; (void)n_in; (void)n_out; (void)y;
...
void linear_iq2s_flat_w2a8_pre(const int8_t* x_q8, float scale_x,
                                ...
                                size_t n_in, size_t n_out, float* y) {
#if defined(__ARM_NEON)
    const struct block_iq2_s_t* w = (const struct block_iq2_s_t*)w_iq2s_blocks;
    const size_t n_blocks_per_row = n_in / IQ2_S_BLOCK_ELEMS;
    ...
    for (size_t n = 0; n < n_out; n++) {
        ...
        for (size_t b = 0; b < n_blocks_per_row; b++) {
            const struct block_iq2_s_t* blk  = &row[b];
            const int8_t*        fblk = flat_row + b * IQ2_S_BLOCK_ELEMS;
            if (b + 2 < n_blocks_per_row) {
                __builtin_prefetch(&row[b + 2], 0, 0);
                __builtin_prefetch(flat_row + (b + 2) * IQ2_S_BLOCK_ELEMS, 0, 0);
            }
            ...
            for (int ib32 = 0; ib32 < 8; ib32++) {
                ...
                const int8x16_t x_lo = vld1q_s8(xb + 0);
                const int8x16_t w_lo = vld1q_s8(fblk + ib32 * 32 + 0);
                const int32x4_t d_lo = vdotq_s32(vdupq_n_s32(0), x_lo, w_lo);
                int_acc = vmlaq_n_s32(int_acc, d_lo, s_lo);

                const int8x16_t x_hi = vld1q_s8(xb + 16);
                const int8x16_t w_hi = vld1q_s8(fblk + ib32 * 32 + 16);
                const int32x4_t d_hi = vdotq_s32(vdupq_n_s32(0), x_hi, w_hi);
                int_acc = vmlaq_n_s32(int_acc, d_hi, s_hi);
...
#else
    (void)x_q8; (void)scale_x; (void)w_iq2s_blocks; (void)w_flat;
    (void)n_in; (void)n_out; (void)y;
    fprintf(stderr, "linear_iq2s_flat_w2a8_pre: NEON required\n");
#endif
}
...
void linear_iq2s_flat_w2a8(const float* x, const void* w_iq2s_blocks,
    ...
    int8_t* x_q8 = heap_alloc_array_aligned(int8_t, n_in);
    const float scale_x = quantize_x_int8_sym(x, n_in, x_q8);
    linear_iq2s_flat_w2a8_pre(x_q8, scale_x, w_iq2s_blocks, w_flat, n_in, n_out, y);
    safe_free((void **) &x_q8);
}
...
#else
    (void)x_q8; (void)scale_x; (void)m; (void)w_iq2s_blocks; (void)w_flat;
    (void)n_in; (void)n_out; (void)y;
    fprintf(stderr, "linear_iq2s_flat_w2a8_prefill_pre: NEON required\n");
#endif
}
...
void linear_iq2s_flat_w2a8_prefill(const float* x, const void* w_iq2s_blocks,
    ...
    linear_iq2s_flat_w2a8_prefill_pre(x_q8, scale_x, m, w_iq2s_blocks,
                                       w_flat, n_in, n_out, y);
    safe_free((void **) &x_q8); safe_free((void **) &scale_x);
}
