...
static_assert(sizeof(struct q4k_predecode_block) == 280,
              "q4k_predecode_block layout changed");

static inline const struct q4k_predecode_block *
q4k_predecode_blocks(const void *packed) {
    return (const struct q4k_predecode_block *)
        ((const uint8_t *) packed + sizeof(struct q4k_predecode_header));
}

static inline const struct q4k_predecode_block *
q4k_predecode_ntile4_blocks(const void *packed) {
    return (const struct q4k_predecode_block *)
        ((const uint8_t *) packed + sizeof(struct q4k_predecode_header));
}
...
static inline bool q4k_predecode_valid(const void *packed, size_t n_in, size_t n_out) {
    ...
    return h->magic == Q4K_PREDECODE_MAGIC
...
static inline bool q4k_predecode_ntile4_valid(const void *packed, size_t n_in,
                                               size_t n_out) {
    if (packed == NULL) return false;
    const struct q4k_predecode_header *h = (const struct q4k_predecode_header *) packed;
    return h->magic == Q4K_PREDECODE_NTILE4_MAGIC
        && h->n_in == (uint32_t) n_in
        && h->n_out == (uint32_t) n_out
        && h->n_blocks_per_row == (uint32_t) (n_in / Q4_K_BLOCK_ELEMS)
        && h->block_bytes == sizeof(struct q4k_predecode_block);
...
size_t q4k_predecode_size_bytes(size_t n_in, size_t n_out) {
    if (n_in == 0 || n_out == 0 || n_in % Q4_K_BLOCK_ELEMS != 0) return 0;
    if (n_in / Q4_K_BLOCK_ELEMS > UINT32_MAX || n_in > UINT32_MAX ||
        n_out > UINT32_MAX) {
        return 0;
    }
    const size_t n_blocks = (n_in / Q4_K_BLOCK_ELEMS) * n_out;
    if (n_blocks > (SIZE_MAX - sizeof(struct q4k_predecode_header)) /
                       sizeof(struct q4k_predecode_block)) {
        return 0;
    }
    return sizeof(struct q4k_predecode_header) +
           n_blocks * sizeof(struct q4k_predecode_block);
}
...
int q4k_predecode_pack(const void *w_q4k, size_t n_in, size_t n_out, void *dst) {
    if (w_q4k == NULL || dst == NULL) return -1;
    ...
    *h = (struct q4k_predecode_header) {
    ...
    struct q4k_predecode_block *dstb = (struct q4k_predecode_block *)
...
int q4k_predecode_ntile4_pack(const void *w_q4k, size_t n_in, size_t n_out,
                              void *dst) {
    if (w_q4k == NULL || dst == NULL) return -1;
...
static inline void q4k_subpair_dots(const uint8_t* q, const float* x1, const float* x2,
...
#else
        int32_t sum = 0;
        for (int j = 0; j < 32; j++) sum += x_q8[s * 32 + j];
        sum32[s] = sum;
#endif
    }
    return scale;
...
void quantize_x_for_q4k_blocks(const float* x, size_t n, int8_t* x_q8,
                                int32_t* sum32, float* scale_blocks) {
    if (x == NULL || x_q8 == NULL || sum32 == NULL || scale_blocks == NULL) return;
    if (n % Q4_K_BLOCK_ELEMS != 0) return;
    const size_t n_blocks = n / Q4_K_BLOCK_ELEMS;
    for (size_t b = 0; b < n_blocks; b++) {
        const size_t elem_off = b * Q4_K_BLOCK_ELEMS;
        scale_blocks[b] = quantize_x_int8_sym(x + elem_off, Q4_K_BLOCK_ELEMS,
                                               x_q8 + elem_off);
...
static inline void q4k_decode_one_row(size_t n, const struct q4k_decode_ctx *c) {
    ...
    float acc = 0.0f;
    if (n + 1 < c->n_out) __builtin_prefetch(row + c->n_blocks_per_row, 0, 0);
    for (size_t b = 0; b < c->n_blocks_per_row; b++) {
        const struct block_q4_K_t* blk = &row[b];
        if (b + 2 < c->n_blocks_per_row) __builtin_prefetch(&row[b + 2], 0, 0);
        ...
        for (int is = 0; is < 8; is += 2, q += 32, xb += 64) {
            ...
            for (int half = 0; half < 32; half += 16) {
                ...
                int32x4_t  d_lo = vdotq_s32(vdupq_n_s32(0), lo,
                                             vld1q_s8(xb + half));
                int32x4_t  d_hi = vdotq_s32(vdupq_n_s32(0), hi,
                                             vld1q_s8(xb + 32 + half));
        ...
        for (int is = 0; is < 8; is += 2, q += 32, xb += 64) {
            ...
            sumi += (int32_t) scales[is + 0] * acc1
                  + (int32_t) scales[is + 1] * acc2;
        }
#endif
...
void linear_q4k_decode_w4a8_pre(const int8_t* x_q8, float scale_x, const int32_t* sum32,
                                 const void* w_q4k, size_t n_in, size_t n_out, float* y) {
    const struct q4k_decode_ctx ctx = {
        .w = (const struct block_q4_K_t *) w_q4k,
        .x_q8 = x_q8, .sum32 = sum32, .scale_x = scale_x, .y = y,
    ...
    if (omp_in_parallel()) {
        ...
        for (size_t n = 0; n < n_out; n++) q4k_decode_one_row(n, &ctx);
    } else if (n_out >= 4096) {
...
static void q4k_pp_pair_row(size_t n, void *vctx) {
    ...
    if (n < c->c0->n_out) q4k_decode_one_row(n, c->c0);
    if (n < c->c1->n_out) q4k_decode_one_row(n, c->c1);
}

void linear_q4k_decode_w4a8(const float* x, const void* w_q4k,
    ...
    union { float f; uint32_t u; } s0, s1, s2, s3, s4;
    s0.f = x[0];
    s1.f = x[n_in / 4];
    s2.f = x[n_in / 2];
    s3.f = x[(3 * n_in) / 4];
    s4.f = x[n_in - 1];
    const uint64_t sig = (uint64_t) s0.u
                       ^ ((uint64_t) s1.u <<  7)
                       ^ ((uint64_t) s2.u << 13)
                       ^ ((uint64_t) s3.u << 19)
                       ^ ((uint64_t) s4.u << 23);
...
void linear_q4k_decode_w4a8_pair(const float* x,
                                  ...
                                  const void* w1_q4k,
                                  size_t n_in,
                                  size_t n_out0, size_t n_out1,
                                  float* y0, float* y1) {
    if (x == NULL || w0_q4k == NULL || w1_q4k == NULL ||
        y0 == NULL || y1 == NULL ||
    ...
    const struct q4k_decode_ctx c0 = {
        ...
        .x_q8 = tl_x_q8, .sum32 = tl_sum32, .scale_x = scale_x, .y = y0,
        .n_blocks_per_row = n_in / Q4_K_BLOCK_ELEMS,
        .n_out = n_out0,
    };
    const struct q4k_decode_ctx c1 = {
        .w = (const struct block_q4_K_t *) w1_q4k,
        .x_q8 = tl_x_q8, .sum32 = tl_sum32, .scale_x = scale_x, .y = y1,
        .n_blocks_per_row = n_in / Q4_K_BLOCK_ELEMS,
        .n_out = n_out1,
    };
    ...
    if (omp_in_parallel()) {
        ...
        for (size_t n = 0; n < n_out_max; n++) {
            if (n < n_out0) q4k_decode_one_row(n, &c0);
            if (n < n_out1) q4k_decode_one_row(n, &c1);
        }
...
void linear_q4k_w4a8_prefill_pre(const int8_t* x_q8, const float* scale_x,
    ...
    for (size_t p = 0; p < n_panels; p++) {
        ...
        for (size_t i = 0; i < m; i++)
            for (size_t r = 0; r < nc; r++) ytile[i * (size_t) NC + r] = 0.0f;
            ...
            for (size_t r = 0; r < nc; r++) {
                ...
                for (; i + MT <= m; i += MT) {
                    ...
                    for (int t = 0; t < MT; t++) {
                        ...
                        int32_t mc = 0;
                        for (int j = 0; j < 8; j++) mc += (int32_t) mins[j] * sp[j];
                        ytile[ii * (size_t) NC + r] += d_blk * (float) vaddvq_s32(acc[t]) - dmin_blk * (float) mc;
                    }
                }
                for (; i < m; i++) {
                    ...
                    int32_t mc = 0;
                    for (int j = 0; j < 8; j++) mc += (int32_t) mins[j] * sp[j];
                    ytile[i * (size_t) NC + r] += d_blk * (float) vaddvq_s32(acc) - dmin_blk * (float) mc;
                }
            }
        }
        for (size_t i = 0; i < m; i++)
    ...
    (void)w_q4k; (void)n_in; (void)n_out; (void)y;
    fprintf(stderr, "linear_q4k_w4a8_prefill_pre: NEON required\n");
#endif
}

void linear_q4k_w4a8_prefill_predecoded(const int8_t* x_q8, const float* scale_x,
                                         const int32_t* sum32, size_t m,
                                         const void* packed, size_t n_in,
                                         size_t n_out, float* y) {
#if defined(__ARM_NEON)
    if (m == 0 || m > GEIST_QUANT_M_CAP) return;
    if (!q4k_predecode_valid(packed, n_in, n_out)) return;
        ...
        for (size_t b = 0; b < n_blocks_per_row; b++) {
            ...
            for (int is = 0; is < 8; is++) {
                ...
                for (size_t i = 0; i < m; i++) {
                    ...
                    const int32_t dot = dot16_i8(xb +  0, q0)
                                      + dot16_i8(xb + 16, q1);
                    sumi[i] += sc * dot;
...
#else
    (void)x_q8; (void)scale_x; (void)sum32; (void)m;
    (void)packed; (void)n_in; (void)n_out; (void)y;
...
void linear_q4k_w4a8_prefill_predecoded_mtile4(const int8_t* x_q8,
    ...
    if (!q4k_predecode_valid(packed, n_in, n_out)) return;
    if (m < 4) {
        linear_q4k_w4a8_prefill_predecoded(x_q8, scale_x, sum32, m,
                                             packed, n_in, n_out, y);
        return;
    }
        ...
        for (; mt < m; mt++) {
            ...
            for (size_t b = 0; b < n_blocks_per_row; b++) {
                ...
                for (int is = 0; is < 8; is++) {
                    ...
                    mins_corr += (int32_t) blk->mins[is] *
                                  sum32[mt * n_chunks + sum_idx];
                }
                acc += sx * (blk->d * (float) sumi -
                             blk->dmin * (float) mins_corr);
            }
            y[mt * n_out + n] = acc;
        }
    }
...
void linear_q4k_w4a8_prefill_predecoded_mtile8(const int8_t* x_q8,
    ...
    if (!q4k_predecode_valid(packed, n_in, n_out)) return;
    if (m < 8) {
        linear_q4k_w4a8_prefill_predecoded_mtile4(x_q8, scale_x, sum32, m,
                                                   packed, n_in, n_out, y);
        return;
    }
...
#define Q4K_ACC_ROW(NR, A0, A1, A2, A3) do {                                      \
                    const struct q4k_predecode_block *blk = blks[(NR)];           \
                    const float d = blk->d;                                        \
                    const float dmin = blk->dmin;                                  \
                    (A0) += sx0 * (d * (float) sumi[(NR)][0] -                    \
                                   dmin * (float) minc[(NR)][0]);                 \
                    (A1) += sx1 * (d * (float) sumi[(NR)][1] -                    \
                                   dmin * (float) minc[(NR)][1]);                 \
                    (A2) += sx2 * (d * (float) sumi[(NR)][2] -                    \
                                   dmin * (float) minc[(NR)][2]);                 \
                    (A3) += sx3 * (d * (float) sumi[(NR)][3] -                    \
                                   dmin * (float) minc[(NR)][3]);                 \
                } while (0)
...
void linear_q4k_w4a8_prefill_predecoded_mtile4_ntile4_packed(
    const int8_t* x_q8, const float* scale_x, const int32_t* sum32,
    size_t m, const void* packed, size_t n_in, size_t n_out, float* y) {
#if defined(__ARM_NEON)
    if (m == 0 || m > GEIST_QUANT_M_CAP) return;
    if (!q4k_predecode_ntile4_valid(packed, n_in, n_out)) return;
...
#define Q4K_ACC_ROW_PACKED(NR, A0, A1, A2, A3) do {                               \
                    if ((NR) < valid_nr) {                                         \
                        const struct q4k_predecode_block *blk = blks + (NR);       \
                        const float d = blk->d;                                    \
                        const float dmin = blk->dmin;                              \
...
#undef Q4K_ACC_ROW_PACKED
        ...
        for (; mt < m; mt++) {
            for (size_t nr = 0; nr < valid_nr; nr++) {
                ...
                for (size_t b = 0; b < n_blocks_per_row; b++) {
                    ...
                    acc += scale_x[mt] * (blk->d * (float) sumi -
                                          blk->dmin * (float) mins_corr);
                }
    ...
    (void)packed; (void)n_in; (void)n_out; (void)y;
    fprintf(stderr,
            "linear_q4k_w4a8_prefill_predecoded_mtile4_ntile4_packed: NEON required\n");
#endif
}
...
void linear_q4k_w4a8_prefill_predecoded_mtile8_ntile4_packed(
    const int8_t* x_q8, const float* scale_x, const int32_t* sum32,
    size_t m, const void* packed, size_t n_in, size_t n_out, float* y) {
#if defined(__ARM_NEON)
    if (m == 0 || m > GEIST_QUANT_M_CAP) return;
...
#define Q4K_ACC_8(NR, A0, A1, A2, A3, A4, A5, A6, A7) do {                    \
                    if ((NR) < valid_nr) {                                     \
                        const struct q4k_predecode_block *blk = blks + (NR);   \
                        const float d = blk->d;                                \
                        const float dmin = blk->dmin;                          \
                        ...
                        (A3) += sx3 * (d * (float) sumi[(NR)][3] -            \
                                       dmin * (float) minc[(NR)][3]);         \
                        (A4) += sx4 * (d * (float) sumi[(NR)][4] -            \
                                       dmin * (float) minc[(NR)][4]);         \
                        (A5) += sx5 * (d * (float) sumi[(NR)][5] -            \
                                       dmin * (float) minc[(NR)][5]);         \
                        (A6) += sx6 * (d * (float) sumi[(NR)][6] -            \
                                       dmin * (float) minc[(NR)][6]);         \
                        (A7) += sx7 * (d * (float) sumi[(NR)][7] -            \
                                       dmin * (float) minc[(NR)][7]);         \
                    }                                                          \
                } while (0)
...
#undef Q4K_ACC_8
            ...
            if (valid_nr > 0) {
                y0[0]=a00; y1[0]=a10; y2[0]=a20; y3[0]=a30;
                y4[0]=a40; y5[0]=a50; y6[0]=a60; y7[0]=a70;
            }
            if (valid_nr > 1) {
                y0[1]=a01; y1[1]=a11; y2[1]=a21; y3[1]=a31;
                y4[1]=a41; y5[1]=a51; y6[1]=a61; y7[1]=a71;
            }
            if (valid_nr > 2) {
                y0[2]=a02; y1[2]=a12; y2[2]=a22; y3[2]=a32;
                y4[2]=a42; y5[2]=a52; y6[2]=a62; y7[2]=a72;
            }
            if (valid_nr > 3) {
                y0[3]=a03; y1[3]=a13; y2[3]=a23; y3[3]=a33;
                y4[3]=a43; y5[3]=a53; y6[3]=a63; y7[3]=a73;
            }
        }
...
#define Q4K_ACC_4(NR, A0, A1, A2, A3) do {                                    \
                    if ((NR) < valid_nr) {                                     \
                        const struct q4k_predecode_block *blk = blks + (NR);   \
                        const float d = blk->d;                                \
                        const float dmin = blk->dmin;                          \
...
void linear_q4k_w4a8_prefill_pair_predecoded_mtile4_ntile4_packed(
    const int8_t* x_q8, const float* scale_x, const int32_t* sum32,
    size_t m, const void* packed0, const void* packed1,
    size_t n_in, size_t n_out, float* y0, float* y1) {
#if defined(__ARM_NEON)
    if (m == 0 || m > GEIST_QUANT_M_CAP) return;
    ...
    for (size_t tt = 0; tt < total_tiles; tt++) {
        ...
        const struct q4k_predecode_block *w =
            q4k_predecode_ntile4_blocks(packed);
        const size_t valid_nr = (nt * 4 + 4 <= n_out) ? 4 : (n_out - nt * 4);
        const struct q4k_predecode_block *tile =
            w + nt * n_blocks_per_row * 4;
        ...
        for (; mt + 4 <= m; mt += 4) {
            ...
            for (size_t b = 0; b < n_blocks_per_row; b++) {
                ...
                for (int is = 0; is < 8; is++) {
                    const size_t xb_off = b * Q4_K_BLOCK_ELEMS +
                                          (size_t) is * 32;
                    const size_t sum_idx = xb_off / 32;
                    const int8_t *xb0 = x_q8 + (mt + 0) * n_in + xb_off;
                    const int8_t *xb1 = x_q8 + (mt + 1) * n_in + xb_off;
                    ...
                    for (size_t nr = 0; nr < valid_nr; nr++) {
                        ...
                        const int32_t sc = (int32_t) blk->scales[is];
                        const int32_t mn = (int32_t) blk->mins[is];
                        const int32_t dot0 = dot16_i8(xb0 + 0, q0) +
                                             dot16_i8(xb0 + 16, q1);
                        const int32_t dot1 = dot16_i8(xb1 + 0, q0) +
                                             dot16_i8(xb1 + 16, q1);
                        const int32_t dot2 = dot16_i8(xb2 + 0, q0) +
                                             dot16_i8(xb2 + 16, q1);
                        const int32_t dot3 = dot16_i8(xb3 + 0, q0) +
                                             dot16_i8(xb3 + 16, q1);
                        sumi[nr][0] += sc * dot0;
                        sumi[nr][1] += sc * dot1;
                        sumi[nr][2] += sc * dot2;
...
#define Q4K_PAIR_ACC_ROW(NR, A0, A1, A2, A3) do {                                \
                    if ((NR) < valid_nr) {                                        \
                        const struct q4k_predecode_block *blk = blks + (NR);      \
                        const float d = blk->d;                                   \
                        const float dmin = blk->dmin;                             \
...
#undef Q4K_PAIR_ACC_ROW
            ...
            if (valid_nr > 0) { yy0[0] = a00; yy1[0] = a10; yy2[0] = a20; yy3[0] = a30; }
            if (valid_nr > 1) { yy0[1] = a01; yy1[1] = a11; yy2[1] = a21; yy3[1] = a31; }
            if (valid_nr > 2) { yy0[2] = a02; yy1[2] = a12; yy2[2] = a22; yy3[2] = a32; }
            if (valid_nr > 3) { yy0[3] = a03; yy1[3] = a13; yy2[3] = a23; yy3[3] = a33; }
        }
...
#else
    (void)x_q8; (void)scale_x; (void)sum32; (void)m;
    (void)packed0; (void)packed1; (void)n_in; (void)n_out;
    (void)y0; (void)y1;
    fprintf(stderr,
...
static inline void q4k_raw_tile4_mtile4_compute(
    const int8_t *x_q8, const float *scale_x, const int32_t *sum32,
    size_t m, const struct block_q4_K_t *w, size_t n_in, size_t n_out,
    size_t nt, float *y) {
#if defined(__ARM_NEON)
    const size_t n_blocks_per_row = n_in / Q4_K_BLOCK_ELEMS;
    const size_t n_chunks = n_in / 32;
    ...
    for (size_t nr = 0; nr < valid_nr; nr++) {
        for (size_t mi = 0; mi < m; mi++) acc[nr][mi] = 0.0f;
    }
    ...
    for (size_t b = 0; b < n_blocks_per_row; b++) {
        ...
        for (size_t nr = 0; nr < valid_nr; nr++) {
            ...
            for (int ip = 0; ip < 4; ip++, q += 32) {
                ...
                for (int half = 0; half < 32; half += 16) {
                    ...
                    const int8x16_t lo =
                        vreinterpretq_s8_u8(vandq_u8(qv, vdupq_n_u8(0x0F)));
                    const int8x16_t hi =
                        vreinterpretq_s8_u8(vshrq_n_u8(qv, 4));
                    for (size_t mi = 0; mi < m; mi++) {
                        const int8_t *xb = x_q8 + mi * n_in + xb_off +
                                           (size_t) half;
                        sumi[nr][mi] += sc0 * dot16_i8(xb, lo) +
                                        sc1 * dot16_i8(xb + 32, hi);
                    }
                }
            }
        }
        ...
        for (size_t nr = 0; nr < valid_nr; nr++) {
            ...
            for (size_t mi = 0; mi < m; mi++) {
                acc[nr][mi] += scale_x[mi] *
                    (d * (float) sumi[nr][mi] -
                     dmin * (float) minc[nr][mi]);
            }
        }
    }
...
#else
    (void)x_q8; (void)scale_x; (void)sum32; (void)m;
    (void)w; (void)n_in; (void)n_out; (void)nt; (void)y;
#endif
}

void linear_q4k_w4a8_prefill_pair_raw_mtile4_ntile4(
    const int8_t* x_q8, const float* scale_x, const int32_t* sum32,
    size_t m, const void* w0_q4k, const void* w1_q4k,
    size_t n_in, size_t n_out, float* y0, float* y1) {
#if defined(__ARM_NEON)
    if (m == 0 || m > GEIST_QUANT_M_CAP || n_out == 0 ||
        x_q8 == NULL || scale_x == NULL || sum32 == NULL ||
        w0_q4k == NULL || w1_q4k == NULL || y0 == NULL || y1 == NULL) {
        return;
    }
...
#endif
    for (size_t nt = 0; nt < n_tiles; nt++) {
        q4k_raw_tile4_mtile4_compute(x_q8, scale_x, sum32, m,
                                     w0, n_in, n_out, nt, y0);
        q4k_raw_tile4_mtile4_compute(x_q8, scale_x, sum32, m,
                                     w1, n_in, n_out, nt, y1);
    }
#else
    (void)x_q8; (void)scale_x; (void)sum32; (void)m;
    (void)w0_q4k; (void)w1_q4k; (void)n_in; (void)n_out;
    (void)y0; (void)y1;
    fprintf(stderr,
            "linear_q4k_w4a8_prefill_pair_raw_mtile4_ntile4: NEON required\n");
#endif
}
...
#else
    (void)x_q8; (void)scale_blocks; (void)sum32; (void)m;
    (void)packed; (void)n_in; (void)n_out; (void)y;
    fprintf(stderr, "linear_q4k_w4a8_prefill_predecoded_mtile4_bscale: NEON required\n");
#endif
}

void linear_q4k_w4a8_prefill(const float* x, const void* w_q4k, size_t m,
    ...
    if (x_q8 == NULL || sum32 == NULL || scale_x == NULL) {
        safe_free((void **) &x_q8); safe_free((void **) &sum32); safe_free((void **) &scale_x);
        return;
    }
    for (size_t i = 0; i < m; i++) {
        scale_x[i] = quantize_x_for_q4k(x + i * n_in, n_in,
                                         x_q8 + i * n_in,
                                         sum32 + i * (n_in / 32));
    }
...
void linear_q4k_decode_w4a8_predecoded(const float* x, const void* packed,
                                        size_t n_in, size_t n_out, float* y) {
    if (!q4k_predecode_valid(packed, n_in, n_out)) return;
    ...
    linear_q4k_w4a8_prefill_predecoded(x_q8, &scale_x, sum32, 1,
                                         packed, n_in, n_out, y);
    safe_free((void **) &x_q8);
    safe_free((void **) &sum32);
}

void linear_q4k_decode_fp32(const float* x, const void* w_q4k,
                             size_t n_in, size_t n_out,
                             float* y) {
    const struct block_q4_K_t* w = (const struct block_q4_K_t*)w_q4k;
    const size_t n_blocks_per_row = n_in / Q4_K_BLOCK_ELEMS;
