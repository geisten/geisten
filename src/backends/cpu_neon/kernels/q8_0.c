...
void linear_q8_0_w8a8_prefill_pre(const int8_t* x_q8, const float* scale_x,
    ...
    for (size_t n = 0; n < n_out; n++) {
        const struct block_q8_0_t* row = w + n * nb_per_row;
        if (n + 1 < n_out) __builtin_prefetch(row + nb_per_row, 0, 0);
        ...
        for (size_t b = 0; b < nb_per_row; b++) {
            const struct block_q8_0_t* blk = &row[b];
            if (b + 2 < nb_per_row) __builtin_prefetch(&row[b + 2], 0, 0);
            const float d = fp16_to_fp32(blk->d);
            ...
            for (size_t i = 0; i < m; i++) {
                ...
                int32_t int_dot = dot16_i8(xb,      qv0)
                                + dot16_i8(xb + 16, qv1);
    ...
    (void)w_q8; (void)n_in; (void)n_out; (void)y;
