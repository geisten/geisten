...
void linear_q5k_decode_w5a8_pre(const int8_t* x_q8, float scale_x, const int32_t* sum32,
                                 const void* w_q5k, size_t n_in, size_t n_out, float* y) {
        ...
        for (size_t b = 0; b < n_blocks_per_row; b++) {
            ...
            for (int is = 0; is < 8; is += 2, ql += 32, xb += 64, sump += 2) {
                ...
                for (int half = 0; half < 32; half += 16) {
                    ...
                    int8x16_t  q_lo = vreinterpretq_s8_u8(vaddq_u8(lo4, vandq_u8(m_lo, vdupq_n_u8(16))));
                    int8x16_t  q_hi = vreinterpretq_s8_u8(vaddq_u8(hi4, vandq_u8(m_hi, vdupq_n_u8(16))));
...
void linear_q5k_decode_w5a8(const float* x, const void* w_q5k,
    ...
    safe_free((void **) &x_q8); safe_free((void **) &sum32);
}
    ...
    for (size_t n = 0; n < n_out; n++) {
        ...
        for (size_t b = 0; b < n_blocks_per_row; b++) {
            ...
            for (int is = 0; is < 8; is += 2, ql += 32) {
                ...
                for (size_t i = 0; i < m; i++) { acc1[i] = 0; acc2[i] = 0; }
                ...
                for (int half = 0; half < 32; half += 16) {
                    ...
                    int8x16_t  q_lo = vreinterpretq_s8_u8(vaddq_u8(lo4, vandq_u8(m_lo, vdupq_n_u8(16))));
                    int8x16_t  q_hi = vreinterpretq_s8_u8(vaddq_u8(hi4, vandq_u8(m_hi, vdupq_n_u8(16))));
                    const size_t xb_off = b * Q5_K_BLOCK_ELEMS + (size_t)(is/2) * 64 + (size_t)half;
        ...
        safe_free((void **) &accs); safe_free((void **) &acc1); safe_free((void **) &acc2);
    }
#else
    (void)x_q8; (void)scale_x; (void)sum32; (void)m;
    (void)w_q5k; (void)n_in; (void)n_out; (void)y;
