...
static inline int32x4_t q3k_w3a8_acc32(int32x4_t acc,
                                         uint8x16_t qs_shifted_lo, uint8x16_t qs_shifted_hi,
                                         uint8x16_t hm_shifted_lo, uint8x16_t hm_shifted_hi,
                                         const int8_t* xb_g32,
                                         int8_t s_first16, int8_t s_second16) {
    const uint8x16_t mask3 = vdupq_n_u8(0x03);
    const uint8x16_t mask1 = vdupq_n_u8(0x01);
    const int8x16_t  bias4 = vdupq_n_s8(4);
    uint8x16_t low2_l = vandq_u8(qs_shifted_lo, mask3);
...
void linear_q3k_decode_w3a8_pre(const int8_t* x_q8, float scale_x,
    ...
    for (size_t n = 0; n < n_out; n++) {
        ...
        for (size_t b = 0; b < n_blocks_per_row; b++) {
            ...
            int_acc = q3k_w3a8_acc32(int_acc, qsA_lo, qsA_hi,
                                     hm_lo, hm_hi,
                                     xb + 0,   sc[0],  sc[1]);
            int_acc = q3k_w3a8_acc32(int_acc, vshrq_n_u8(qsA_lo, 2), vshrq_n_u8(qsA_hi, 2),
                                     vshrq_n_u8(hm_lo,  1), vshrq_n_u8(hm_hi,  1),
                                     xb + 32,  sc[2],  sc[3]);
            int_acc = q3k_w3a8_acc32(int_acc, vshrq_n_u8(qsA_lo, 4), vshrq_n_u8(qsA_hi, 4),
                                     vshrq_n_u8(hm_lo,  2), vshrq_n_u8(hm_hi,  2),
                                     xb + 64,  sc[4],  sc[5]);
            int_acc = q3k_w3a8_acc32(int_acc, vshrq_n_u8(qsA_lo, 6), vshrq_n_u8(qsA_hi, 6),
                                     vshrq_n_u8(hm_lo,  3), vshrq_n_u8(hm_hi,  3),
                                     xb + 96,  sc[6],  sc[7]);
            int_acc = q3k_w3a8_acc32(int_acc, qsB_lo, qsB_hi,
                                     vshrq_n_u8(hm_lo,  4), vshrq_n_u8(hm_hi,  4),
                                     xb + 128, sc[8],  sc[9]);
            int_acc = q3k_w3a8_acc32(int_acc, vshrq_n_u8(qsB_lo, 2), vshrq_n_u8(qsB_hi, 2),
                                     vshrq_n_u8(hm_lo,  5), vshrq_n_u8(hm_hi,  5),
                                     xb + 160, sc[10], sc[11]);
            int_acc = q3k_w3a8_acc32(int_acc, vshrq_n_u8(qsB_lo, 4), vshrq_n_u8(qsB_hi, 4),
                                     vshrq_n_u8(hm_lo,  6), vshrq_n_u8(hm_hi,  6),
                                     xb + 192, sc[12], sc[13]);
            int_acc = q3k_w3a8_acc32(int_acc, vshrq_n_u8(qsB_lo, 6), vshrq_n_u8(qsB_hi, 6),
                                     vshrq_n_u8(hm_lo,  7), vshrq_n_u8(hm_hi,  7),
                                     xb + 224, sc[14], sc[15]);
...
#else
    (void)x_q8; (void)scale_x; (void)w_q3k; (void)n_in; (void)n_out; (void)y;
...
void linear_q3k_w3a8_prefill_pre(const int8_t* x_q8, const float* scale_x, size_t m,
    ...
    for (size_t n = 0; n < n_out; n++) {
        ...
        for (size_t i = 0; i < m; i++) my_accs[i] = 0.0f;
        ...
        for (size_t b = 0; b < n_blocks_per_row; b++) {
            ...
            for (size_t i = 0; i < m; i++) block_int_accs[i] = 0;

#define PROCESS_GROUP_M(G, QSL, QSH, HML, HMH, XOFF) do {                       \
    int8x16_t q_l, q_h;                                                          \
    ...
    for (size_t i = 0; i < m; i++) {                                             \
        ...
        block_int_accs[i] += s_first * dot_l + s_second * dot_h;                 \
            ...
            PROCESS_GROUP_M(1, vshrq_n_u8(qsA_lo, 2), vshrq_n_u8(qsA_hi, 2),
                                vshrq_n_u8(hm_lo,  1), vshrq_n_u8(hm_hi,  1), 32);
            PROCESS_GROUP_M(2, vshrq_n_u8(qsA_lo, 4), vshrq_n_u8(qsA_hi, 4),
                                vshrq_n_u8(hm_lo,  2), vshrq_n_u8(hm_hi,  2), 64);
            PROCESS_GROUP_M(3, vshrq_n_u8(qsA_lo, 6), vshrq_n_u8(qsA_hi, 6),
                                vshrq_n_u8(hm_lo,  3), vshrq_n_u8(hm_hi,  3), 96);
            PROCESS_GROUP_M(4, qsB_lo, qsB_hi,
                                vshrq_n_u8(hm_lo,  4), vshrq_n_u8(hm_hi,  4), 128);
            PROCESS_GROUP_M(5, vshrq_n_u8(qsB_lo, 2), vshrq_n_u8(qsB_hi, 2),
                                vshrq_n_u8(hm_lo,  5), vshrq_n_u8(hm_hi,  5), 160);
            PROCESS_GROUP_M(6, vshrq_n_u8(qsB_lo, 4), vshrq_n_u8(qsB_hi, 4),
                                vshrq_n_u8(hm_lo,  6), vshrq_n_u8(hm_hi,  6), 192);
            PROCESS_GROUP_M(7, vshrq_n_u8(qsB_lo, 6), vshrq_n_u8(qsB_hi, 6),
...
#undef PROCESS_GROUP_M
        ...
        for (size_t i = 0; i < m; i++) y[i * n_out + n] = my_accs[i];
        safe_free((void **) &my_accs); safe_free((void **) &block_int_accs);
    }
#else
    (void)x_q8; (void)scale_x; (void)m; (void)w_q3k; (void)n_in; (void)n_out; (void)y;
