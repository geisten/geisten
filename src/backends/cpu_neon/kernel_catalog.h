...
struct cpu_neon_kernel_entry {
    ...
    cpu_neon_isa_mask requires;
    void (*linear_m1)(const float *, const struct geist_weight *,
                      struct geist_backend *, float *);
    void (*linear_mN)(const float *, const struct geist_weight *, size_t,
                      struct geist_backend *, float *);
    const char       *name;
};
