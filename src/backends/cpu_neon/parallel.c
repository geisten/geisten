...
static inline void do_chunk(int w, int N, size_t n_total,
                             geist_pp_body_fn fn, void *ctx) {
    const size_t chunk = (n_total + (size_t) N - 1) / (size_t) N;
    const size_t lo    = (size_t) w * chunk;
    if (lo >= n_total) return;
    const size_t hi    = (lo + chunk > n_total) ? n_total : lo + chunk;
    for (size_t i = lo; i < hi; i++) fn(i, ctx);
}

static void do_dynamic_chunks(size_t n_total, size_t grain,
                              geist_pp_body_fn fn, void *ctx);
...
static void *worker_main(void *arg) {
    ...
    for (;;) {
        ...
        uint64_t spin = 0;
        for (;;) {
            epoch = atomic_load_explicit(&g_state.task_epoch,
                                          memory_order_acquire);
            if (epoch != last_seen) break;
            if (atomic_load_explicit(&g_state.shutdown,
                                      memory_order_relaxed)) return NULL;
            if (++spin >= GEIST_PP_SPIN_BUDGET) {
                ...
                atomic_fetch_add_explicit(&g_state.sleeping, 1,
                                           memory_order_release);
                pthread_mutex_lock(&g_state.wait_mutex);
                while (atomic_load_explicit(&g_state.task_epoch,
                                              memory_order_acquire)
                       == last_seen
                       && !atomic_load_explicit(&g_state.shutdown,
                                                 memory_order_relaxed)) {
                    pthread_cond_wait(&g_state.wait_cv, &g_state.wait_mutex);
                }
                pthread_mutex_unlock(&g_state.wait_mutex);
                atomic_fetch_sub_explicit(&g_state.sleeping, 1,
                                           memory_order_release);
                spin = 0;
            }
        }
        ...
        if (g_state.dynamic_chunks) {
            do_dynamic_chunks(g_state.n_total, g_state.chunk_size,
                              g_state.body_fn, g_state.ctx);
        } else {
            do_chunk(w, g_state.n_threads,
                     g_state.n_total, g_state.body_fn, g_state.ctx);
        }
        atomic_fetch_sub_explicit(&g_state.pending_workers, 1,
                                   memory_order_release);
    }
}
...
static int detect_thread_count(void) {
    ...
    if (env) {
        ...
        if (n > 0) return n > GEIST_PP_MAX_THREADS ? GEIST_PP_MAX_THREADS : n;
    }
    /* Fallback: physical cores. */
#if defined(__APPLE__)
    int ncpu = 0; size_t sz = sizeof(ncpu);
    if (sysctlbyname("hw.perflevel0.physicalcpu", &ncpu, &sz, NULL, 0) == 0
        && ncpu > 0) {
    ...
    if (n < 1) n = 4;
#endif
    if (n > GEIST_PP_MAX_THREADS) n = GEIST_PP_MAX_THREADS;
    return (int) n;
}

static void init_pool(void) {
    g_state.n_threads = detect_thread_count();
    if (g_state.n_threads <= 1) return;   /* no workers; master does all */
    for (int w = 1; w < g_state.n_threads; w++) {
        g_state.worker_ids[w] = w;
        if (pthread_create(&g_state.workers[w], NULL, worker_main,
                            &g_state.worker_ids[w]) != 0) {
...
void geist_pp_parallel_for(size_t n, geist_pp_body_fn body_fn, void *ctx) {
    ...
    if (N == 1 || n <= 1) {
        for (size_t i = 0; i < n; i++) body_fn(i, ctx);
        return;
    }
    /* Publish task. */
    g_state.n_total = n;
    ...
    while (atomic_load_explicit(&g_state.pending_workers,
                                  memory_order_acquire) != 0) {
        /* spin */
    }
}
...
static void do_dynamic_chunks(size_t n_total, size_t grain,
    ...
    if (grain == 0) { grain = 1; }
    for (;;) {
        const size_t lo = atomic_fetch_add_explicit(&g_state.next_index, grain,
                                                    memory_order_relaxed);
        if (lo >= n_total) { return; }
        size_t hi = lo + grain;
        if (hi > n_total) { hi = n_total; }
        for (size_t i = lo; i < hi; i++) { fn(i, ctx); }
    }
}
