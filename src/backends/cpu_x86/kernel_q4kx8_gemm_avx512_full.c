...
#define AVX512_TARGET \
    "avx2,avx,f16c,fma,avx512f,avx512bw,avx512dq,avx512vl"

/* Load 16 fp16 values from two 8-fp16 sources into 16 fp32 in a __m512.
 * Mirrors GGML_F32Cx8x2_LOAD in llama.cpp's repack.cpp. */
__attribute__((target(AVX512_TARGET)))
static inline __m512 ggml_f32cx8x2_load(const uint16_t *x, const uint16_t *y) {
...
__attribute__((target(AVX512_TARGET)))
static void q4kx8_gemm16x16_tile_avx512(size_t                     nb,
    ...
    for (size_t b = 0; b < nb; b++) {
        ...
        for (int sb = 0; sb < 4; sb++) { /* QK_K / 64 = 4 */
            ...
            const __m256i mins_and_scales_0 = _mm256_set_epi32(
                    (int) utmp_10[3], (int) utmp_10[2], (int) utmp_10[1], (int) utmp_10[0],
                    (int) utmp_00[3], (int) utmp_00[2], (int) utmp_00[1], (int) utmp_00[0]);
            const __m512i scales_0 = _mm512_cvtepu8_epi16(
                    _mm256_unpacklo_epi8(mins_and_scales_0, mins_and_scales_0));

            const __m256i mins_and_scales_1 = _mm256_set_epi32(
                    (int) utmp_11[3], (int) utmp_11[2], (int) utmp_11[1], (int) utmp_11[0],
                    (int) utmp_01[3], (int) utmp_01[2], (int) utmp_01[1], (int) utmp_01[0]);
            const __m512i scales_1 = _mm512_cvtepu8_epi16(
                    _mm256_unpacklo_epi8(mins_and_scales_1, mins_and_scales_1));

            const __m512i mins_01 = _mm512_cvtepu8_epi16(_mm256_unpacklo_epi8(
                    _mm256_shuffle_epi32(mins_and_scales_0, 78),
            ...
            for (int rp = 0; rp < 4; rp++) {
                ...
                for (int r = 0; r < 4; r++) {
                    for (int gi = 0; gi < 4; gi++) {
                        bsums_buf[r * 4 + gi] =
                                a_ptrs[rp][b].bsums[r * 16 + sb * 4 + gi];
                    }
                }
                __m256i lhs_bsums_0123_01 =
                        _mm256_loadu_si256((const __m256i *) bsums_buf);
                ...
                acc_rows[rp * 4 + 0] = _mm512_fmadd_ps(_mm512_cvtepi32_ps(iacc_row_0), _mm512_mul_ps(col_scale_f32, _mm512_shuffle_ps(row_scale_f32, row_scale_f32,   0)), acc_rows[rp * 4 + 0]);
                ...
                acc_min_rows[rp * 4 + 0] = _mm512_fmadd_ps(_mm512_cvtepi32_ps(iacc_row_min_0), _mm512_mul_ps(col_dmin_f32, _mm512_shuffle_ps(row_scale_f32, row_scale_f32,   0)), acc_min_rows[rp * 4 + 0]);
