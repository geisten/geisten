...
void rope_compute(size_t seq_len, size_t head_dim, size_t n_rotated_dims,
                  float theta, float* cos_out, float* sin_out) {
    ...
    if (n_active > half) n_active = half;
...
void rope_apply(float* x, const float* cos, const float* sin,
...
void rope_compute_at(size_t pos_offset, size_t n_positions, size_t head_dim,
                     size_t n_rotated_dims, float theta,
    ...
    for (size_t i = n_active; i < half; i++) inv_freq[i] = 0.0f;
...
void attention_mqa_causal_kv(const float* q, const float* k, const float* v,
    ...
    for (size_t t = 0; t < n_q; t++) {
        ...
        for (size_t h = 0; h < n_q_heads; h++) {
            ...
            for (size_t s = s_lo; s <= s_hi; s++) {
                ...
                float dot = 0.0f;
                for (size_t i = 0; i < head_dim; i++) dot += qv[i] * kv_kh[i];
                float sc = dot * scale;
                scores[s] = sc;
            ...
            for (size_t s = s_lo; s <= s_hi; s++) {
                ...
                for (size_t i = 0; i < head_dim; i++) outv[i] += w * vv[i];
            }
        }
    }
...
void attention_mqa_causal(const float* q, const float* k, const float* v,
                          size_t seq_len, size_t n_q_heads, size_t n_kv_heads,
                          size_t head_dim, size_t sliding_window,
                          float* out) {
...
void rmsnorm_fp32(const float* x, const float* weight,
                  size_t n_rows, size_t hidden, float eps,
                  float* y) {
    for (size_t r = 0; r < n_rows; r++) {
        ...
        if (weight) {
            ...
            for (; j < hidden; j++) yr[j] = xr[j] * rsqrt * weight[j];
        } else {
            for (; j + 16 <= hidden; j += 16) {
                float32x4_t x0 = vld1q_f32(xr + j + 0);
                float32x4_t x1 = vld1q_f32(xr + j + 4);
            ...
            for (; j < hidden; j++) yr[j] = xr[j] * rsqrt;
        }
#else
        if (weight) {
            for (size_t i = 0; i < hidden; i++) yr[i] = xr[i] * rsqrt * weight[i];
        } else {
            for (size_t i = 0; i < hidden; i++) yr[i] = xr[i] * rsqrt;
        }
#endif
    }
}
