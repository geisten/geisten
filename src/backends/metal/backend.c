...
static const char metal_attn_source[] =
    ...
    " threadgroup float scores[1024];uint r=tg.x,h=tg.y;if(r>=p.rows||h>=p.q_heads){return;}uint kvh=h/(p.q_heads/p.kv_heads);uint qpos=p.q_position+r;uint slo=(p.sliding_window>0u&&qpos+1u>p.sliding_window)?qpos+1u-p.sliding_window:0u;uint shi=qpos<p.kv_len?qpos:p.kv_len-1u;uint n=shi>=slo?shi-slo+1u:0u;float mx=-3.402823466e+38f;for(uint si=lid;si<n;si+=256u){uint t=slo+si;float dot=0.0f;for(uint i=0u;i<p.head_dim;i++){dot+=q[p.q_offset+r*p.q_heads*p.head_dim+h*p.head_dim+i]*kc[p.k_cache_offset+t*p.kv_heads*p.head_dim+kvh*p.head_dim+i];}scores[si]=dot;mx=dot>mx?dot:mx;}threadgroup_barrier(mem_flags::mem_threadgroup);threadgroup float red[256];red[lid]=mx;threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=128u;st>0u;st>>=1u){if(lid<st){red[lid]=red[lid]>red[lid+st]?red[lid]:red[lid+st];}threadgroup_barrier(mem_flags::mem_threadgroup);}mx=red[0];float ss=0.0f;for(uint si=lid;si<n;si+=256u){float e=exp(scores[si]-mx);scores[si]=e;ss+=e;}red[lid]=ss;threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=128u;st>0u;st>>=1u){if(lid<st){red[lid]+=red[lid+st];}threadgroup_barrier(mem_flags::mem_threadgroup);}float inv=1.0f/red[0];for(uint i=lid;i<p.head_dim;i+=256u){float acc=0.0f;for(uint si=0u;si<n;si++){uint t=slo+si;acc+=scores[si]*inv*vc[p.v_cache_offset+t*p.kv_heads*p.head_dim+kvh*p.head_dim+i];}y[p.y_offset+r*p.q_heads*p.head_dim+h*p.head_dim+i]=acc;}\n"
    "}\n";
...
static const char metal_attn_f16_source[] =
    ...
    " threadgroup float scores[1024];uint r=tg.x,h=tg.y;if(r>=p.rows||h>=p.q_heads){return;}uint kvh=h/(p.q_heads/p.kv_heads);uint qpos=p.q_position+r;uint slo=(p.sliding_window>0u&&qpos+1u>p.sliding_window)?qpos+1u-p.sliding_window:0u;uint shi=qpos<p.kv_len?qpos:p.kv_len-1u;uint n=shi>=slo?shi-slo+1u:0u;float mx=-3.402823466e+38f;for(uint si=lid;si<n;si+=256u){uint t=slo+si;float dot=0.0f;for(uint i=0u;i<p.head_dim;i++){dot+=q[p.q_offset+r*p.q_heads*p.head_dim+h*p.head_dim+i]*float(kc[p.k_cache_offset+t*p.kv_heads*p.head_dim+kvh*p.head_dim+i]);}scores[si]=dot;mx=dot>mx?dot:mx;}threadgroup_barrier(mem_flags::mem_threadgroup);threadgroup float red[256];red[lid]=mx;threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=128u;st>0u;st>>=1u){if(lid<st){red[lid]=red[lid]>red[lid+st]?red[lid]:red[lid+st];}threadgroup_barrier(mem_flags::mem_threadgroup);}mx=red[0];float ss=0.0f;for(uint si=lid;si<n;si+=256u){float e=exp(scores[si]-mx);scores[si]=e;ss+=e;}red[lid]=ss;threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=128u;st>0u;st>>=1u){if(lid<st){red[lid]+=red[lid+st];}threadgroup_barrier(mem_flags::mem_threadgroup);}float inv=1.0f/red[0];for(uint i=lid;i<p.head_dim;i+=256u){float acc=0.0f;for(uint si=0u;si<n;si++){uint t=slo+si;acc+=scores[si]*inv*float(vc[p.v_cache_offset+t*p.kv_heads*p.head_dim+kvh*p.head_dim+i]);}y[p.y_offset+r*p.q_heads*p.head_dim+h*p.head_dim+i]=acc;}\n"
    "}\n";
...
static const char metal_attn_qnorm_source[] =
    ...
    " threadgroup float scores[1024];threadgroup float red[256];threadgroup float qv[512];uint r=tg.x,h=tg.y;if(r>=p.rows||h>=p.qh||p.hd>512u){return;}uint base=nr.xo+r*nr.xs+h*nr.hd;float ssq=0.0f;for(uint i=lid;i<nr.hd;i+=256u){float v=q[base+i];ssq+=v*v;}red[lid]=ssq;threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=128u;st>0u;st>>=1u){if(lid<st){red[lid]+=red[lid+st];}threadgroup_barrier(mem_flags::mem_threadgroup);}float invn=rsqrt(red[0]/float(nr.hd)+nr.eps);uint hd2=nr.hd/2u,rr=nr.ro+r;for(uint i=lid;i<hd2;i+=256u){float x0=q[base+i]*invn*qw[nr.wo+i],x1=q[base+i+hd2]*invn*qw[nr.wo+i+hd2];float co=c[nr.co+rr*nr.rs+i],si=s[nr.so+rr*nr.rs+i];qv[i]=x0*co-x1*si;qv[i+hd2]=x0*si+x1*co;}threadgroup_barrier(mem_flags::mem_threadgroup);uint kvh=h/(p.qh/p.kvh),qp=p.qpos+r;uint slo=(p.sw>0u&&qp+1u>p.sw)?qp+1u-p.sw:0u;uint shi=qp<p.kv_len?qp:p.kv_len-1u;uint n=shi>=slo?shi-slo+1u:0u;float mx=-3.402823466e+38f;for(uint si=lid;si<n;si+=256u){uint t=slo+si;float dot=0.0f;for(uint i=0u;i<p.hd;i++){dot+=qv[i]*kc[p.kco+t*p.kvh*p.hd+kvh*p.hd+i];}scores[si]=dot;mx=dot>mx?dot:mx;}red[lid]=mx;threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=128u;st>0u;st>>=1u){if(lid<st){red[lid]=red[lid]>red[lid+st]?red[lid]:red[lid+st];}threadgroup_barrier(mem_flags::mem_threadgroup);}mx=red[0];float sm=0.0f;for(uint si=lid;si<n;si+=256u){float e=exp(scores[si]-mx);scores[si]=e;sm+=e;}red[lid]=sm;threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=128u;st>0u;st>>=1u){if(lid<st){red[lid]+=red[lid+st];}threadgroup_barrier(mem_flags::mem_threadgroup);}float invs=1.0f/red[0];for(uint i=lid;i<p.hd;i+=256u){float acc=0.0f;for(uint si=0u;si<n;si++){uint t=slo+si;acc+=scores[si]*invs*vc[p.vco+t*p.kvh*p.hd+kvh*p.hd+i];}y[p.yo+r*p.qh*p.hd+h*p.hd+i]=acc;}\n"
    "}\n";
...
static const char metal_attn_qnorm_f16_source[] =
    ...
    " threadgroup float scores[1024];threadgroup float red[256];threadgroup float qv[512];uint r=tg.x,h=tg.y;if(r>=p.rows||h>=p.qh||p.hd>512u){return;}uint base=nr.xo+r*nr.xs+h*nr.hd;float ssq=0.0f;for(uint i=lid;i<nr.hd;i+=256u){float v=q[base+i];ssq+=v*v;}red[lid]=ssq;threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=128u;st>0u;st>>=1u){if(lid<st){red[lid]+=red[lid+st];}threadgroup_barrier(mem_flags::mem_threadgroup);}float invn=rsqrt(red[0]/float(nr.hd)+nr.eps);uint hd2=nr.hd/2u,rr=nr.ro+r;for(uint i=lid;i<hd2;i+=256u){float x0=q[base+i]*invn*qw[nr.wo+i],x1=q[base+i+hd2]*invn*qw[nr.wo+i+hd2];float co=c[nr.co+rr*nr.rs+i],si=s[nr.so+rr*nr.rs+i];qv[i]=x0*co-x1*si;qv[i+hd2]=x0*si+x1*co;}threadgroup_barrier(mem_flags::mem_threadgroup);uint kvh=h/(p.qh/p.kvh),qp=p.qpos+r;uint slo=(p.sw>0u&&qp+1u>p.sw)?qp+1u-p.sw:0u;uint shi=qp<p.kv_len?qp:p.kv_len-1u;uint n=shi>=slo?shi-slo+1u:0u;float mx=-3.402823466e+38f;for(uint si=lid;si<n;si+=256u){uint t=slo+si;float dot=0.0f;for(uint i=0u;i<p.hd;i++){dot+=qv[i]*float(kc[p.kco+t*p.kvh*p.hd+kvh*p.hd+i]);}scores[si]=dot;mx=dot>mx?dot:mx;}red[lid]=mx;threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=128u;st>0u;st>>=1u){if(lid<st){red[lid]=red[lid]>red[lid+st]?red[lid]:red[lid+st];}threadgroup_barrier(mem_flags::mem_threadgroup);}mx=red[0];float sm=0.0f;for(uint si=lid;si<n;si+=256u){float e=exp(scores[si]-mx);scores[si]=e;sm+=e;}red[lid]=sm;threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=128u;st>0u;st>>=1u){if(lid<st){red[lid]+=red[lid+st];}threadgroup_barrier(mem_flags::mem_threadgroup);}float invs=1.0f/red[0];for(uint i=lid;i<p.hd;i+=256u){float acc=0.0f;for(uint si=0u;si<n;si++){uint t=slo+si;acc+=scores[si]*invs*float(vc[p.vco+t*p.kvh*p.hd+kvh*p.hd+i]);}y[p.yo+r*p.qh*p.hd+h*p.hd+i]=acc;}\n"
    "}\n";
...
[[nodiscard]] static enum geist_status metal_ensure_q4k_pipeline(
    ...
    void *f32_source = metal_msg_send_id_cstr(
        st, ns_string, "stringWithUTF8String:", metal_f32_source);
...
[[nodiscard]] static enum geist_status metal_attention(
    ...
    if (k_rows > 1024u) {
        geist_backend_set_error(be, GEIST_E_UNSUPPORTED,
                                "metal attention: kv length exceeds shader limit");
        return GEIST_E_UNSUPPORTED;
    }
...
[[nodiscard]] static enum geist_status metal_ple_block(
    ...
    if (!metal_tensor_is_f32_matrix(block->hidden, &hidden_rows,
                                    &hidden_cols, &hidden_offset,
                                    &hidden_stride) ||
        !metal_tensor_is_f32_matrix(block->per_layer_input, &ple_rows,
                                    &ple_cols, &ple_offset, &ple_stride) ||
...
[[nodiscard]] static enum geist_status metal_attention_query_block(
    ...
    if (block->d_model == 0 || block->q_heads == 0 ||
        ...
        block->q_heads % block->kv_heads != 0 ||
        (block->head_dim & 1u) != 0 ||
        block->q_position >= block->kv_len || block->kv_len > 1024u ||
        block->post_attn_norm_weight == nullptr ||
        block->post_attn_scratch == nullptr) {
        return GEIST_E_INVALID_ARG;
    }
...
[[nodiscard]] static enum geist_status metal_create(
    ...
    *st = (struct metal_state){0};
    ...
    st->use_q6k_nt4 = q6k_nt4 == nullptr || strcmp(q6k_nt4, "0") != 0;
