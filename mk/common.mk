...
BACKEND_CONFIG_HEADER := $(BUILD_DIR)/geist_backend_config.h

CFLAGS        := $(CFLAGS_BASE) -include $(BACKEND_CONFIG_HEADER) \
...
ifneq ($(filter vulkan,$(BACKENDS)),)
VULKAN_CFLAGS ?= -I/opt/homebrew/include -I/usr/local/include
CFLAGS += $(VULKAN_CFLAGS)
CFLAGS_STRICT += $(VULKAN_CFLAGS)
LIB_SOURCES += \
    src/backends/vulkan/backend.c
ifeq ($(shell uname -s),Linux)
LDLIBS += -ldl
endif
endif

ifneq ($(filter metal,$(BACKENDS)),)
LIB_SOURCES += \
...
$(BACKEND_CONFIG_HEADER): FORCE
	@mkdir -p $(@D)
	@tmp="$@.tmp"; \
	{ \
	    printf '%s\n' '/* generated by mk/common.mk; do not edit */'; \
	    printf '#define GEIST_BUILD_BACKENDS "%s"\n' '$(strip $(BACKENDS))'; \
	} > "$$tmp"; \
...
$(BUILD_DIR)/src/%.o: src/%.c $(BACKEND_CONFIG_HEADER)
	@mkdir -p $(@D)
	$(CC) $(CFLAGS_STRICT) -MMD -MP -c $< -o $@

$(BUILD_DIR)/%.o: %.c $(BACKEND_CONFIG_HEADER)
	@mkdir -p $(@D)
...
$(BUILD_DIR)/ext/%.o: %.c $(BACKEND_CONFIG_HEADER)
	@mkdir -p $(@D)
	$(CC) $(CFLAGS) -MMD -MP -c $< -o $@
