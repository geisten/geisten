...
static void pack_i2_s(const int8_t *trits, size_t n_in, size_t n_out,
                      float scale, uint8_t *buf) {
...
static float ref_dot_i2s(const int8_t *trits, float scale,
    ...
    int64_t dot = 0;
    for (size_t i = 0; i < n_in; i++) dot += (int64_t) trits[i] * xq[i];
    free(xq);
...
int main(void) {
    ...
    for (size_t i = 0; i < n_in * n_out; i++) trits[i] = (int8_t) ((rand() % 3) - 1);
    ...
    /* --- M=1 decode --- */
    {
        ...
        for (size_t i = 0; i < n_in; i++) x[i] = ((rand() % 2001) - 1000) * 0.01f;
        ...
        for (size_t r = 0; r < n_out; r++) {
            ...
            if (rel > 1e-4f) { if (mm < 5) printf("m1 MISMATCH r=%zu ref=%.6f got=%.6f rel=%.2e\n", r, ref, y[r], rel); mm++; }
        }
    ...
    for (size_t m = 1; m <= 20; m += (m == 1 ? 7 : 3)) {  /* 1,8,11,14,17,20 — mt16/mt8/mt4/scalar */
        ...
        for (size_t t = 0; t < m; t++) {
            for (size_t r = 0; r < n_out; r++) {
                ...
                if (rel > 1e-4f) { if (mm < 5) printf("mN m=%zu MISMATCH t=%zu r=%zu ref=%.6f got=%.6f rel=%.2e\n", m, t, r, ref, got, rel); mm++; }
            }
        }
    ...
    /* --- Fused F16 lm-head GEMV --- */
    {
        ...
        for (size_t i = 0; i < fn_in; i++) x[i] = ((rand() % 2001) - 1000) * 0.01f;
        float *y = malloc(fn_out * sizeof(float));
        struct geist_weight wf = {0};
        wf.raw = Wf; wf.n_in = (int32_t) fn_in; wf.n_out = (int32_t) fn_out;
