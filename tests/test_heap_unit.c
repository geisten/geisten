...
int main(void) {
    ...
    CHECK(((uintptr_t) p & (OPTIMAL_ALIGNMENT - 1)) == 0,
          "alloc not OPTIMAL_ALIGNMENT-aligned");
    ...
    CHECK(heap_alloc_aligned(64, 96) == nullptr,
          "non-pow2 align>OPTIMAL must be null");

    /* Overflow: rounding SIZE_MAX up to alignment would wrap to a tiny
     * value. Must be refused, not under-allocate. */
    CHECK(heap_alloc_aligned(SIZE_MAX, 64) == nullptr,
          "SIZE_MAX alloc must be refused");
    CHECK(heap_alloc_aligned(SIZE_MAX - 16, 64) == nullptr,
          "near-SIZE_MAX alloc must be refused");
    ...
    CHECK(heap_calloc_aligned(SIZE_MAX, 2, 64) == nullptr,
          "calloc overflow must be refused");
    ...
    CHECK(((uintptr_t) a1 & (OPTIMAL_ALIGNMENT - 1)) == 0,
          "arena alloc not OPTIMAL-aligned");
    CHECK((uintptr_t) a2 > (uintptr_t) a1, "arena allocs must advance");

    /* Non-pow2 alignment refused. */
    CHECK(arena_allocate_aligned(&arena, 16, 48) == nullptr,
          "arena non-pow2 align must be null");
