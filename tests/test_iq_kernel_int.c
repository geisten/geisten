...
int main(void) {
    ...
    if (iq2s != nullptr) {
        ...
        cblas_sgemv(CblasRowMajor_,
                    ...
                    y_ref,
                    1);
        linear_iq2s_flat_w2a8(x, iq2s->data, flat, n_in, n_out, y_fast);
        const float cos = cosine_similarity(y_ref, y_fast, n_out);
        printf("  cos=%.6f  y_ref[0..3]=%.3f %.3f %.3f  y_fast[0..3]=%.3f %.3f %.3f\n",
               cos,
               y_ref[0],
               y_ref[1],
               y_ref[2],
    ...
    if (iq3s != nullptr) {
        ...
        cblas_sgemv(CblasRowMajor_,
                    ...
                    y_ref,
                    1);
        linear_iq3s_flat_w3a8(x, iq3s->data, flat, n_in, n_out, y_fast);
        const float cos = cosine_similarity(y_ref, y_fast, n_out);
        printf("  cos=%.6f  y_ref[0..3]=%.3f %.3f %.3f  y_fast[0..3]=%.3f %.3f %.3f\n",
               cos,
               y_ref[0],
    ...
    /* ---- M>1 flat-prefill kernels ---- */
    {
        const size_t m_values_flat[] = {2, 4, 8};
        for (size_t k = 0; k < sizeof m_values_flat / sizeof m_values_flat[0]; k++) {
            ...
            if (iq2s != nullptr) {
                ...
                cblas_sgemm(CblasRowMajor_,
                            ...
                            (int) n_out);
                free(w_fp32);
                linear_iq2s_flat_w2a8_prefill(x, iq2s->data, flat, m, n_in, n_out, y_fast);
                float min_cos = 1.0f;
            ...
            if (iq3s != nullptr) {
                ...
                cblas_sgemm(CblasRowMajor_,
                            ...
                            (int) n_out);
                free(w_fp32);
                linear_iq3s_flat_w3a8_prefill(x, iq3s->data, flat, m, n_in, n_out, y_fast);
                float min_cos = 1.0f;
                for (size_t i = 0; i < m; i++) {
                    const float c = cosine_similarity(y_ref + i * n_out, y_fast + i * n_out, n_out);
                    if (c < min_cos)
